...
# 3. Importer uniquement les calibrations mesurees de cet exemplaire, puis
# provisionner la calibration Elliptic deja sauvegardee dans persist. Aucun
# blob propre a l'appareil n'est livre dans le paquet.
/usr/libexec/hotdog-import-factory-calibration.py || true
/usr/libexec/hotdog-elliptic-calibration.py || true

# 4. Le service de fichiers du DSP. Le PD capteurs ne se cree qu'au premier
...
/usr/libexec/hotdog-run-hexagonrpcd
echo "hexagonrpcd a $(uptime_s)s"

# 5. Attendre que SEE publie l'accelerometre.
i=0
while [ "$i" -lt 40 ]; do
	if timeout 5 /usr/libexec/hotdog-ssc-client.py accel 2>/dev/null | grep -q "data-type"; then
...
# 6. Le demon, retire du niveau par defaut : c'est ici qu'il demarre.
rc-service iio-sensor-proxy start

# 7. Ne rendre la main qu'une fois l'enumeration terminee. C'est tout l'objet de
