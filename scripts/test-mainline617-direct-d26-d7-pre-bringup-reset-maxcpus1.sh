...
BOOT_IMAGE="$HOTDOG_ROOT/images/pmos-experiments/2026-07-13-121700-mainline617-direct-d26-d7-pre-bringup-reset-maxcpus1/boot.img"
...
BOOT_SHA=3e04e8dd8284bc372e2a55330b888dc691bef28b05a2fc2db94f465468e697af
...
if [ "${1:-}" = -h ] || [ "${1:-}" = --help ]; then
	cat <<'USAGE'
Usage: test-mainline617-direct-d26-d7-pre-bringup-reset-maxcpus1.sh

Request PSCI SYSTEM_RESET after idle/hotplug thread setup and immediately
before bringup_nonboot_cpus(), with maxcpus=1. Rollback restores verified R6
plus the stock DTBO.
USAGE
	exit 0
fi

[ "$#" -eq 0 ] || die "This pinned D26 launcher accepts no options" 2
...
check_sha "D26 maxcpus=1 pre-bringup reset image" "$BOOT_IMAGE" "$BOOT_SHA"
