...
BOOT_IMAGE="$HOTDOG_ROOT/images/pmos-experiments/2026-07-13-124100-mainline617-direct-d28-d7-post-bringup-reset-maxcpus0/boot.img"
...
BOOT_SHA=91da81ba552753ef14b5e3f3bb074a3e02dfa9c26edb23e042f8951b21fee5da
...
if [ "${1:-}" = -h ] || [ "${1:-}" = --help ]; then
	cat <<'USAGE'
Usage: test-mainline617-direct-d28-d7-post-bringup-reset-maxcpus0.sh

Request PSCI SYSTEM_RESET immediately after bringup_nonboot_cpus() with
maxcpus=0. Reaching the reset proves the call is skipped successfully and
locates the remaining hang later in smp_init(). Rollback restores verified R6
plus the stock DTBO.
USAGE
	exit 0
fi

[ "$#" -eq 0 ] || die "This pinned D28 launcher accepts no options" 2
...
check_sha "D28 maxcpus=0 post-bringup reset image" "$BOOT_IMAGE" "$BOOT_SHA"
