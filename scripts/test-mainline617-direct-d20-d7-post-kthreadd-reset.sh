...
BOOT_IMAGE="$HOTDOG_ROOT/images/pmos-experiments/2026-07-13-110000-mainline617-direct-d20-d7-post-kthreadd-reset/boot.img"
...
BOOT_SHA=4d64de88f338f9f985dec1270d696a69dd7f5fb00b09741e3b556776f80ee42b
...
if [ "${1:-}" = -h ] || [ "${1:-}" = --help ]; then
	cat <<'USAGE'
Usage: test-mainline617-direct-d20-d7-post-kthreadd-reset.sh

Keep D9's complete image contract but request PSCI SYSTEM_RESET after PID 1
observes kthreadd completion, immediately before kernel_init_freeable().
Rollback restores verified R6 plus the stock DTBO.
USAGE
	exit 0
fi

[ "$#" -eq 0 ] || die "This pinned D20 launcher accepts no options" 2
...
check_sha "D20 post-kthreadd reset image" "$BOOT_IMAGE" "$BOOT_SHA"
