...
usage() {
	cat <<'USAGE'
Usage: test-mainline617-direct-d1-pack.sh [safe test-boot-b-image.sh options]
	...
	while [ "$#" -gt 0 ]; do
		case "$1" in
			...
			--poll=*|--fastboot-timeout=*|--rescue-watch-timeout=*|--rescue-watch-poll=*)
				...
				;;
			--*)
				die "Unsupported option for pinned D1-pack test: $1" 2
				;;
			*)
				die "Unsupported positional argument for pinned D1-pack test: $1" 2
				;;
		esac
	done
}
