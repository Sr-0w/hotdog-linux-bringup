...
BOOT_IMAGE="$HOTDOG_ROOT/images/pmos-experiments/2026-08-01-095300-mainline617-direct-native-ufs-g3-v2-lane-after-host-reset/boot.img"
...
BOOT_SHA=7374d7cb3d74a0fe59e9fc560217451d09cd0f4fc2169a5dcd242bf574356737
...
if [ "${1:-}" = -h ] || [ "${1:-}" = --help ]; then
	cat <<'USAGE'
Usage: test-mainline617-direct-d14-ufs-after-host-reset.sh
...
This D14 image retains D13's seven SM8150 PCS Gear 3 writes, revision-2 TX/RX
lane calibration, corrected runtime identity, HS-G3 bootstrap limit, and
controller trace. Its only functional change pulses the attached UFS device
reset after QCOM's final host-controller reset and PHY calibration, immediately
before HCE is enabled. This corrects the observed cold-boot ordering difference:
mainline otherwise resets the device before its final host reset, while the
working downstream path resets the host before the device. The trace records
CFG1, controller status, and reset-GPIO state around the pulse. The DTB,
filtered DTBO, wrapped pmOS initramfs, command line, watchdogs, and rollback
images are byte-identical to D13.
