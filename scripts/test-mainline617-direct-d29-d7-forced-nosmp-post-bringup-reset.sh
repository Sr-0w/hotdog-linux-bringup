...
BOOT_IMAGE="$HOTDOG_ROOT/images/pmos-experiments/2026-07-13-130600-mainline617-direct-d29-d7-forced-nosmp-post-bringup-reset/boot.img"
...
BOOT_SHA=8a245da635c575512231a3899fc3deef124564d7ffcaedb915d5ed251fbfed4a
...
if [ "${1:-}" = -h ] || [ "${1:-}" = --help ]; then
	cat <<'USAGE'
Usage: test-mainline617-direct-d29-d7-forced-nosmp-post-bringup-reset.sh

Force setup_max_cpus=0 in the kernel and request PSCI SYSTEM_RESET immediately
after bringup_nonboot_cpus(). Rollback restores verified R6 plus stock DTBO.
USAGE
	exit 0
fi

[ "$#" -eq 0 ] || die "This pinned D29 launcher accepts no options" 2
...
check_sha "D29 forced no-SMP post-bringup reset image" "$BOOT_IMAGE" "$BOOT_SHA"
