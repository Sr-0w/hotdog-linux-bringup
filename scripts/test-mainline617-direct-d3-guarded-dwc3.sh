...
BOOT_SHA=c5ade1ffaa458fe6943fda13c208c5e6df9ce3f5e2af8dec8cc7c599fc72ea30
...
if [ "${1:-}" = -h ] || [ "${1:-}" = --help ]; then
	cat <<'USAGE'
Usage: test-mainline617-direct-d3-guarded-dwc3.sh

Run the direct Linux 6.17 DWC3 diagnostic with the D3 no-op DTBO. The no-op
overlay preserves the embedded mainline DTB byte-for-byte, including its
two-cell /soc address format.

The image adds three bounded recovery mechanisms without changing the DWC3
probe:

  - CONFIG_QCOM_WDT=y provides /dev/watchdog0 before PID 1 arms its deadline.
  - PM8150 PON mode-bootloader=2 lets RESTART2 select Fastboot.
  - The static PID 1 supervisor shortens the hardware watchdog to two seconds
    before requesting the bootloader restart at its 300-second deadline.

The eleven large userspace cells retain the prior meaning. Cells 6-10 report
DWC3 diagnostic entry, a6f8800.usb creation, QCOM wrapper binding, HS PHY
binding, and a600000.usb core binding.

This launcher is hash-pinned, uses a dual boot_b/dtbo_b transaction, starts
detached restore watchers, and accepts no options. Set HOTDOG_TEST_START_MODE
to pmos-ssh (default) or fastboot before invoking it.
USAGE
	exit 0
fi

[ "$#" -eq 0 ] || die "This pinned guarded D3 launcher accepts no options" 2
...
check_sha "guarded direct mainline image" "$BOOT_IMAGE" "$BOOT_SHA"
