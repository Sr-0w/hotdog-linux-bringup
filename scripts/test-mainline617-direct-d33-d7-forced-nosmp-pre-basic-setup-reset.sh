...
BOOT_IMAGE="$HOTDOG_ROOT/images/pmos-experiments/2026-07-13-141500-mainline617-direct-d33-d7-forced-nosmp-pre-basic-setup-reset/boot.img"
...
BOOT_SHA=f193e6fd59a8ec3d050e3f4ca9b523da4a6705c3c0e02d3e31d29f0b1635acbc
...
if [ "${1:-}" = -h ] || [ "${1:-}" = --help ]; then
	cat <<'USAGE'
Usage: test-mainline617-direct-d33-d7-forced-nosmp-pre-basic-setup-reset.sh

Force setup_max_cpus=0 and request PSCI SYSTEM_RESET before do_basic_setup().
Rollback restores verified R6 plus stock DTBO.
USAGE
	exit 0
fi

[ "$#" -eq 0 ] || die "This pinned D33 launcher accepts no options" 2
...
check_sha "D33 forced no-SMP pre-basic-setup reset image" "$BOOT_IMAGE" "$BOOT_SHA"
