...
BOOT_IMAGE="$HOTDOG_ROOT/images/pmos-experiments/2026-07-13-113900-mainline617-direct-d24-d7-post-smp-init-reset-maxcpus1/boot.img"
...
BOOT_SHA=5b851b1e04623eb60622c7181e8974653c24c82b0875d31ca352af78877957c1
...
if [ "${1:-}" = -h ] || [ "${1:-}" = --help ]; then
	cat <<'USAGE'
Usage: test-mainline617-direct-d24-d7-post-smp-init-reset-maxcpus1.sh

Repeat D23's reset after smp_init() with maxcpus=1. Reaching the reset proves
that secondary CPU bring-up is the blocking part of smp_init(). Rollback
restores verified R6 plus the stock DTBO.
USAGE
	exit 0
fi

[ "$#" -eq 0 ] || die "This pinned D24 launcher accepts no options" 2
...
check_sha "D24 maxcpus=1 post-smp-init reset image" "$BOOT_IMAGE" "$BOOT_SHA"
