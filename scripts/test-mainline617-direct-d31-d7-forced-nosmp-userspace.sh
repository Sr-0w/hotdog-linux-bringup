...
BOOT_IMAGE="$HOTDOG_ROOT/images/pmos-experiments/2026-07-13-134700-mainline617-direct-d31-d7-forced-nosmp-userspace/boot.img"
...
BOOT_SHA=0af212920af5080daf26a0189d74a6cae834aa43b7b5bcf58d0de93143fe2237
...
if [ "${1:-}" = -h ] || [ "${1:-}" = --help ]; then
	cat <<'USAGE'
Usage: test-mainline617-direct-d31-d7-forced-nosmp-userspace.sh

Boot mainline directly with secondary CPU activation forcibly disabled and no
diagnostic reset checkpoint. Rollback restores verified R6 plus stock DTBO.
USAGE
	exit 0
fi

[ "$#" -eq 0 ] || die "This pinned D31 launcher accepts no options" 2
...
check_sha "D31 forced no-SMP userspace image" "$BOOT_IMAGE" "$BOOT_SHA"
...
exec "$HOTDOG_ROOT/scripts/test-boot-b-image.sh" \
	...
	--restore-after system --boot-wait 360 --poll 2 --fastboot-timeout 15 \
