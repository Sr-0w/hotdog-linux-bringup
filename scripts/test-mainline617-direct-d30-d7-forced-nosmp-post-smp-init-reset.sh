...
BOOT_IMAGE="$HOTDOG_ROOT/images/pmos-experiments/2026-07-13-131300-mainline617-direct-d30-d7-forced-nosmp-post-smp-init-reset/boot.img"
...
BOOT_SHA=8b1802b0e6cea492a8f6ee4bdb6dcac68e73fd9a8179101988fade15c32b11cd
...
if [ "${1:-}" = -h ] || [ "${1:-}" = --help ]; then
	cat <<'USAGE'
Usage: test-mainline617-direct-d30-d7-forced-nosmp-post-smp-init-reset.sh

Force setup_max_cpus=0 and request PSCI SYSTEM_RESET after all of smp_init().
Rollback restores verified R6 plus stock DTBO.
USAGE
	exit 0
fi

[ "$#" -eq 0 ] || die "This pinned D30 launcher accepts no options" 2
...
check_sha "D30 forced no-SMP post-smp-init reset image" "$BOOT_IMAGE" "$BOOT_SHA"
