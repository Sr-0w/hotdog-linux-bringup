...
BOOT_IMAGE="$HOTDOG_ROOT/images/pmos-experiments/2026-07-13-091700-mainline617-direct-d13-d7-primary-switched-reset/boot.img"
...
BOOT_SHA=5c7ca0cb7c77653d74c4e2fbee7af3e88fc568312af5809474e57816ca35b645
...
if [ "${1:-}" = -h ] || [ "${1:-}" = --help ]; then
	cat <<'USAGE'
Usage: test-mainline617-direct-d13-d7-primary-switched-reset.sh

Keep D9's complete image contract but request PSCI SYSTEM_RESET at the start
of __primary_switched, after MMU enable, early mapping and virtual relocation.
Rollback restores verified R6 plus the stock DTBO.
USAGE
	exit 0
fi

[ "$#" -eq 0 ] || die "This pinned D13 launcher accepts no options" 2
...
check_sha "D13 primary-switched reset image" "$BOOT_IMAGE" "$BOOT_SHA"
