...
BOOT_IMAGE="$HOTDOG_ROOT/images/pmos-experiments/2026-07-13-145600-mainline617-direct-d36-d7-forced-nosmp-post-initcall-level3-reset/boot.img"
...
BOOT_SHA=36389555550addbd22290f3902ac08a3227a6feedf2972c15f3eb4476cd308f4
...
if [ "${1:-}" = -h ] || [ "${1:-}" = --help ]; then
	cat <<'USAGE'
Usage: test-mainline617-direct-d36-d7-forced-nosmp-post-initcall-level3-reset.sh

Force setup_max_cpus=0 and request PSCI SYSTEM_RESET after initcall level 3.
Rollback restores verified R6 plus stock DTBO.
USAGE
	exit 0
fi

[ "$#" -eq 0 ] || die "This pinned D36 launcher accepts no options" 2
...
check_sha "D36 forced no-SMP post-initcall-level-3 reset image" "$BOOT_IMAGE" "$BOOT_SHA"
