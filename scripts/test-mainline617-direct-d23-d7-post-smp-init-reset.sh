...
BOOT_IMAGE="$HOTDOG_ROOT/images/pmos-experiments/2026-07-13-112800-mainline617-direct-d23-d7-post-smp-init-reset/boot.img"
...
BOOT_SHA=cc87668f3debee208ff6baefd15d7f3ec218cffc989e885e9d2be25d7440aa98
...
if [ "${1:-}" = -h ] || [ "${1:-}" = --help ]; then
	cat <<'USAGE'
Usage: test-mainline617-direct-d23-d7-post-smp-init-reset.sh

Keep D9's complete image contract but request PSCI SYSTEM_RESET after PID 1
returns from smp_init() and before sched_init_smp(). Rollback restores verified
R6 plus the stock DTBO.
USAGE
	exit 0
fi

[ "$#" -eq 0 ] || die "This pinned D23 launcher accepts no options" 2
...
check_sha "D23 post-smp-init reset image" "$BOOT_IMAGE" "$BOOT_SHA"
