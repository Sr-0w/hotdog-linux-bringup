...
BOOT_IMAGE="$HOTDOG_ROOT/images/pmos-experiments/2026-07-13-090300-mainline617-direct-d12-d7-post-cpu-setup-reset/boot.img"
...
BOOT_SHA=d8ac57e4a4aa8c5cedca6cf90fb2ce9ecef2558f5f15dc01512305981f51dfab
...
if [ "${1:-}" = -h ] || [ "${1:-}" = --help ]; then
	cat <<'USAGE'
Usage: test-mainline617-direct-d12-d7-post-cpu-setup-reset.sh

Keep D9's complete image contract but request PSCI SYSTEM_RESET after initial
idmap/cache handling, init_kernel_el(), and __cpu_setup(), immediately before
__primary_switch. Rollback restores verified R6 plus the stock DTBO.
USAGE
	exit 0
fi

[ "$#" -eq 0 ] || die "This pinned D12 launcher accepts no options" 2
...
check_sha "D12 post-CPU-setup reset image" "$BOOT_IMAGE" "$BOOT_SHA"
