...
BOOT_IMAGE="$HOTDOG_ROOT/images/pmos-experiments/2026-07-15-102057-mainline617-direct-userspace-nosmp-noraid6bench-entrywdtoff/boot.img"
...
BOOT_SHA=38c103f9cc03cc7f0f90033cf29b658e42d73486e0875f24f6dd815ecd7d7ffe
...
if [ "${1:-}" = -h ] || [ "${1:-}" = --help ]; then
	cat <<'USAGE'
Usage: test-mainline617-direct-userspace-nosmp-noraid6bench-entrywdtoff.sh

Boot mainline directly to postmarketOS with the hardware-validated temporary
single-CPU and RAID6 benchmark bypasses, plus early APSS watchdog disable.
Rollback restores verified R6 and the stock DTBO.
USAGE
	exit 0
fi

[ "$#" -eq 0 ] || die "This pinned direct-userspace launcher accepts no options" 2
...
check_sha "direct mainline userspace image" "$BOOT_IMAGE" "$BOOT_SHA"
...
exec "$HOTDOG_ROOT/scripts/test-boot-b-image.sh" \
	...
	--restore-after system --boot-wait 480 --poll 1 --fastboot-timeout 15 \
