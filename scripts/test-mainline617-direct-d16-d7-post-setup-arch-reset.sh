...
BOOT_IMAGE="$HOTDOG_ROOT/images/pmos-experiments/2026-07-13-101700-mainline617-direct-d16-d7-post-setup-arch-reset/boot.img"
...
BOOT_SHA=c4eae792d904b486001802feec7363e416f838552a540d3728f898c1863daf70
...
if [ "${1:-}" = -h ] || [ "${1:-}" = --help ]; then
	cat <<'USAGE'
Usage: test-mainline617-direct-d16-d7-post-setup-arch-reset.sh

Keep D9's complete image contract but request PSCI SYSTEM_RESET immediately
after setup_arch() returns in start_kernel(). Rollback restores verified R6
plus the stock DTBO.
USAGE
	exit 0
fi

[ "$#" -eq 0 ] || die "This pinned D16 launcher accepts no options" 2
...
check_sha "D16 post-setup-arch reset image" "$BOOT_IMAGE" "$BOOT_SHA"
