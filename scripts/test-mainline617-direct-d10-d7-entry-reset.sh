...
BOOT_IMAGE="$HOTDOG_ROOT/images/pmos-experiments/2026-07-13-084100-mainline617-direct-d10-d7-entry-reset/boot.img"
...
BOOT_SHA=67c922b2311a7ac26795d590fa8d054d68ac8c9a5a62704e3d28171d623969a1
...
if [ "${1:-}" = -h ] || [ "${1:-}" = --help ]; then
	cat <<'USAGE'
Usage: test-mainline617-direct-d10-d7-entry-reset.sh

Keep D9's D7-compatible DTB, ramdisk, command line, and boot-image layout, but
replace its kernel with a probe that calls PSCI SYSTEM_RESET at primary_entry.
A changed hardware outcome proves that the bootloader reached mainline code.
Rollback restores the verified R6 no-watchdog bridge and stock DTBO.
USAGE
	exit 0
fi

[ "$#" -eq 0 ] || die "This pinned D10 launcher accepts no options" 2
...
check_sha "D10 entry-reset image" "$BOOT_IMAGE" "$BOOT_SHA"
