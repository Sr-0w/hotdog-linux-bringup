...
BOOT_IMAGE="$HOTDOG_ROOT/images/pmos-experiments/2026-07-13-152000-mainline617-direct-d38-d7-forced-nosmp-post-initcall-level4-reset/boot.img"
...
BOOT_SHA=65b94338086851b0e1c8e883fd0fbd63a9ef5699ceffe64c65c2ab0e210c6141
...
if [ "${1:-}" = -h ] || [ "${1:-}" = --help ]; then
	cat <<'USAGE'
Usage: test-mainline617-direct-d38-d7-forced-nosmp-post-initcall-level4-reset.sh

Force setup_max_cpus=0 and request PSCI SYSTEM_RESET after initcall level 4.
Rollback restores verified R6 plus stock DTBO.
USAGE
	exit 0
fi

[ "$#" -eq 0 ] || die "This pinned D38 launcher accepts no options" 2
...
check_sha "D38 forced no-SMP post-initcall-level-4 reset image" "$BOOT_IMAGE" "$BOOT_SHA"
