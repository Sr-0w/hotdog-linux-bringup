...
BOOT_IMAGE="$HOTDOG_ROOT/images/pmos-experiments/2026-07-13-142600-mainline617-direct-d34-d7-forced-nosmp-post-basic-setup-reset/boot.img"
...
BOOT_SHA=68ff6f261a07d6f647a759195928af47dd0de8a355a69a577d40038bfea19a84
...
if [ "${1:-}" = -h ] || [ "${1:-}" = --help ]; then
	cat <<'USAGE'
Usage: test-mainline617-direct-d34-d7-forced-nosmp-post-basic-setup-reset.sh

Force setup_max_cpus=0 and request PSCI SYSTEM_RESET after do_basic_setup().
Rollback restores verified R6 plus stock DTBO.
USAGE
	exit 0
fi

[ "$#" -eq 0 ] || die "This pinned D34 launcher accepts no options" 2
...
check_sha "D34 forced no-SMP post-basic-setup reset image" "$BOOT_IMAGE" "$BOOT_SHA"
