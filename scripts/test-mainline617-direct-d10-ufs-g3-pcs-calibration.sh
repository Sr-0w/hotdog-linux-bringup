...
BOOT_IMAGE="$HOTDOG_ROOT/images/pmos-experiments/2026-07-31-223339-mainline617-direct-native-ufs-g3-pcs-calibration/boot.img"
...
BOOT_SHA=be39e7f796a101b0b5727dfd7f37970144852690d444d14c739fffe5a5e4ae9e
...
if [ "${1:-}" = -h ] || [ "${1:-}" = --help ]; then
	cat <<'USAGE'
Usage: test-mainline617-direct-d10-ufs-g3-pcs-calibration.sh
...
This D10 image changes only the mainline kernel and returns to the D8 UFS reset
ordering. It adds seven SM8150 PCS G3 calibration writes that are present in
the downstream OnePlus QMP v4 table and already used by the mainline SC7280
QMP v4 table, but are absent from mainline's SM8150 table. The DTB, filtered
DTBO, wrapped pmOS initramfs, command line, watchdogs, and rollback images are
byte-identical to the preceding native-UFS captures.
