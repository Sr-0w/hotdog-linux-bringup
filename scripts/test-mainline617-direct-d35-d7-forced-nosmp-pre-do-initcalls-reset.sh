...
BOOT_IMAGE="$HOTDOG_ROOT/images/pmos-experiments/2026-07-13-143800-mainline617-direct-d35-d7-forced-nosmp-pre-do-initcalls-reset/boot.img"
...
BOOT_SHA=348316717922cd14449ad212b702b5405dc42ad1a1187b0a9e43360c19dee74a
...
if [ "${1:-}" = -h ] || [ "${1:-}" = --help ]; then
	cat <<'USAGE'
Usage: test-mainline617-direct-d35-d7-forced-nosmp-pre-do-initcalls-reset.sh

Force setup_max_cpus=0 and request PSCI SYSTEM_RESET before do_initcalls().
Rollback restores verified R6 plus stock DTBO.
USAGE
	exit 0
fi

[ "$#" -eq 0 ] || die "This pinned D35 launcher accepts no options" 2
...
check_sha "D35 forced no-SMP pre-do-initcalls reset image" "$BOOT_IMAGE" "$BOOT_SHA"
