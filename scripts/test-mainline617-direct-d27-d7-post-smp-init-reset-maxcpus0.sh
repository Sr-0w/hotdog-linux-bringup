...
BOOT_IMAGE="$HOTDOG_ROOT/images/pmos-experiments/2026-07-13-123000-mainline617-direct-d27-d7-post-smp-init-reset-maxcpus0/boot.img"
...
BOOT_SHA=38e6efb5bc0dd9899955819a72d612a40544aff6dd76281f5cd0d8367ba41ced
...
if [ "${1:-}" = -h ] || [ "${1:-}" = --help ]; then
	cat <<'USAGE'
Usage: test-mainline617-direct-d27-d7-post-smp-init-reset-maxcpus0.sh

Repeat the post-smp_init() reset checkpoint with maxcpus=0. Reaching the reset
proves that skipping bringup_nonboot_cpus() allows smp_init() to return.
Rollback restores verified R6 plus the stock DTBO.
USAGE
	exit 0
fi

[ "$#" -eq 0 ] || die "This pinned D27 launcher accepts no options" 2
...
check_sha "D27 maxcpus=0 post-smp-init reset image" "$BOOT_IMAGE" "$BOOT_SHA"
