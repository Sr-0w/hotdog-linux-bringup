...
BOOT_IMAGE="$HOTDOG_ROOT/images/pmos-experiments/2026-07-13-111900-mainline617-direct-d22-d7-pre-smp-init-reset/boot.img"
...
BOOT_SHA=cbc1cfb693e24003d9f94625fbea9a7a9468dc709b465006beefb149f738505f
...
if [ "${1:-}" = -h ] || [ "${1:-}" = --help ]; then
	cat <<'USAGE'
Usage: test-mainline617-direct-d22-d7-pre-smp-init-reset.sh

Keep D9's complete image contract but request PSCI SYSTEM_RESET after PID 1
completes pre-SMP initcalls, immediately before smp_init() starts secondary CPU
bring-up. Rollback restores verified R6 plus the stock DTBO.
USAGE
	exit 0
fi

[ "$#" -eq 0 ] || die "This pinned D22 launcher accepts no options" 2
...
check_sha "D22 pre-smp-init reset image" "$BOOT_IMAGE" "$BOOT_SHA"
