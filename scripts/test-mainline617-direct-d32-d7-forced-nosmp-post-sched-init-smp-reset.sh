...
BOOT_IMAGE="$HOTDOG_ROOT/images/pmos-experiments/2026-07-13-135900-mainline617-direct-d32-d7-forced-nosmp-post-sched-init-smp-reset/boot.img"
...
BOOT_SHA=493d89ccc71c37ebe32966eeaab36997b0abab2a559c4d78ae57ee05be114a33
...
if [ "${1:-}" = -h ] || [ "${1:-}" = --help ]; then
	cat <<'USAGE'
Usage: test-mainline617-direct-d32-d7-forced-nosmp-post-sched-init-smp-reset.sh

Force setup_max_cpus=0 and request PSCI SYSTEM_RESET after sched_init_smp().
Rollback restores verified R6 plus stock DTBO.
USAGE
	exit 0
fi

[ "$#" -eq 0 ] || die "This pinned D32 launcher accepts no options" 2
...
check_sha "D32 forced no-SMP post-sched-init-smp reset image" "$BOOT_IMAGE" "$BOOT_SHA"
