...
BOOT_SHA=f8e83ae15cb016612433b8a2d800d828b025d56c76640a2ebb41a3061baf8994
...
if [ "${1:-}" = -h ] || [ "${1:-}" = --help ]; then
	cat <<'USAGE'
Usage: test-mainline617-direct-d8-d7-dtbo.sh

Pair the exact K1 payload already validated through kexec with the
hardware-validated D7 DTBO. The rollback contract restores R5 while retaining
D7, so this test changes only the boot image relative to the working control.
No command-line override is accepted.
USAGE
	exit 0
fi

[ "$#" -eq 0 ] || die "This pinned D8 launcher accepts no options" 2
...
check_sha "D1 exact K1 boot image" "$BOOT_IMAGE" "$BOOT_SHA"
