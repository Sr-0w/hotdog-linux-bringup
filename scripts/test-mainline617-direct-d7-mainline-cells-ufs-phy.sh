...
BOOT_IMAGE="$HOTDOG_ROOT/images/pmos-experiments/2026-07-31-002500-mainline617-direct-source-init2-dwc3-passive-probe/boot.img"
CANDIDATE_DTBO="$HOTDOG_ROOT/images/pmos-experiments/2026-07-31-004000-d7-mainline-cells-ufs-phy-compatible/dtbo_b-d7-ufs-gdsc-bridge-filtered-drop-fragment-46-drop-fragment-59.img"
...
BOOT_SHA=395417027c0acb0b4496598a7d65e0e42bd94da2fcf6d0d6aff5be0221a28c83
EARLY_BREADCRUMB_PHYS=0x81c0f800
CANDIDATE_DTBO_SHA=4dcd27080f0deb28e7813be7d7b1c907e66094674956088d02becc679a2a6218
...
if [ "${1:-}" = -h ] || [ "${1:-}" = --help ]; then
	cat <<'USAGE'
Usage: test-mainline617-direct-d7-mainline-cells-ufs-phy.sh
...
downstream string `qcom,ufs-phy-qmp-v4`. This candidate therefore changes only
`dtbo_b` again: it keeps D7's useful supply bridge but drops both fragment 46
and fragment 59. Applying the resulting 56-fragment overlay to the exact
embedded DTB preserves the mainline UFS PHY compatible, all required supplies,
the two-cell `/soc@0` format, and valid USB and UFS `reg` properties.
...
without binding a gadget UDC. A separate static supervisor uses a
...
check_sha "D7 mainline-cell UFS PHY candidate dtbo_b" "$CANDIDATE_DTBO" "$CANDIDATE_DTBO_SHA"
