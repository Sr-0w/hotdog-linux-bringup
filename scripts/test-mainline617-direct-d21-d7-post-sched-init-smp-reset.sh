...
BOOT_IMAGE="$HOTDOG_ROOT/images/pmos-experiments/2026-07-13-111200-mainline617-direct-d21-d7-post-sched-init-smp-reset/boot.img"
...
BOOT_SHA=4763ebfe3ba2e33b91d5055c2c80b17c9a69a5ff063ed04bd14a69dc8c2701f7
...
if [ "${1:-}" = -h ] || [ "${1:-}" = --help ]; then
	cat <<'USAGE'
Usage: test-mainline617-direct-d21-d7-post-sched-init-smp-reset.sh

Keep D9's complete image contract but request PSCI SYSTEM_RESET after PID 1
completes SMP bring-up and sched_init_smp(), before the regular initcall
sequence. Rollback restores verified R6 plus the stock DTBO.
USAGE
	exit 0
fi

[ "$#" -eq 0 ] || die "This pinned D21 launcher accepts no options" 2
...
check_sha "D21 post-sched-init-smp reset image" "$BOOT_IMAGE" "$BOOT_SHA"
