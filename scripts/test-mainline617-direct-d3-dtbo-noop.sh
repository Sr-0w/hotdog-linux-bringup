...
if [ "${1:-}" = -h ] || [ "${1:-}" = --help ]; then
  printf '%s\n' 'Usage: test-mainline617-direct-d3-dtbo-noop.sh'
  printf '%s\n' 'Pinned D3: no-op dtbo_b, D1 boot_b, dual rescue, R5 rollback, slot B.'
  exit 0
fi
[ "$#" -eq 0 ] || die "This pinned D3 launcher accepts no options" 2
...
check_sha "D1 boot image" "$BOOT_IMAGE" "$BOOT_SHA"
