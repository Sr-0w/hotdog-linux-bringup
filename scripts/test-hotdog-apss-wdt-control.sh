...
for command_name in od qemu-aarch64 truncate zig; do
...
grep -q '^HOTDOG_APSS_WDT_CONTROL_V2$' "$arm_log"
...
grep -q '^HOTDOG_APSS_WDT_CONTROL_V2$' "$disable_log"
...
grep -q '^HOTDOG_APSS_WDT_CONTROL_V2$' "$disarm_log"
