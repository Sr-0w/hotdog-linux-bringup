...
BOOT_IMAGE="$HOTDOG_ROOT/images/pmos-experiments/2026-07-13-151000-mainline617-direct-d37-d7-forced-nosmp-post-initcall-level5-reset/boot.img"
...
BOOT_SHA=a8bdef4551474097a93dd5cecdaab4b8e45cc66a32f9a680b193145638878f5c
...
if [ "${1:-}" = -h ] || [ "${1:-}" = --help ]; then
	cat <<'USAGE'
Usage: test-mainline617-direct-d37-d7-forced-nosmp-post-initcall-level5-reset.sh

Force setup_max_cpus=0 and request PSCI SYSTEM_RESET after initcall level 5.
Rollback restores verified R6 plus stock DTBO.
USAGE
	exit 0
fi

[ "$#" -eq 0 ] || die "This pinned D37 launcher accepts no options" 2
...
check_sha "D37 forced no-SMP post-initcall-level-5 reset image" "$BOOT_IMAGE" "$BOOT_SHA"
