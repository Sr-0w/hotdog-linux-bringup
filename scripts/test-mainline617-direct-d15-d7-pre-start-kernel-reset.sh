...
BOOT_IMAGE="$HOTDOG_ROOT/images/pmos-experiments/2026-07-13-100500-mainline617-direct-d15-d7-pre-start-kernel-reset/boot.img"
...
BOOT_SHA=0ebd90d0c89b0e3f68fcda0f52c0dd7fb7da387fc8f580244347545051726d41
...
if [ "${1:-}" = -h ] || [ "${1:-}" = --help ]; then
	cat <<'USAGE'
Usage: test-mainline617-direct-d15-d7-pre-start-kernel-reset.sh

Keep D9's complete image contract but request PSCI SYSTEM_RESET after the
complete MMU-on assembly setup, immediately before start_kernel(). Rollback
restores verified R6 plus the stock DTBO.
USAGE
	exit 0
fi

[ "$#" -eq 0 ] || die "This pinned D15 launcher accepts no options" 2
...
check_sha "D15 pre-start-kernel reset image" "$BOOT_IMAGE" "$BOOT_SHA"
