...
BOOT_IMAGE="$HOTDOG_ROOT/images/pmos-experiments/2026-07-13-085300-mainline617-direct-d11-d7-post-idmap-reset/boot.img"
...
BOOT_SHA=9063d8c6bf8741ce0e10834536f2fd87d40a3c7080bb2a3d338656a891528a5f
...
if [ "${1:-}" = -h ] || [ "${1:-}" = --help ]; then
	cat <<'USAGE'
Usage: test-mainline617-direct-d11-d7-post-idmap-reset.sh

Keep D9's complete image contract but request PSCI SYSTEM_RESET only after the
initial ARM64 MMU-state check, boot-argument preservation, early stack setup,
and idmap creation. Rollback restores verified R6 plus the stock DTBO.
USAGE
	exit 0
fi

[ "$#" -eq 0 ] || die "This pinned D11 launcher accepts no options" 2
...
check_sha "D11 post-idmap reset image" "$BOOT_IMAGE" "$BOOT_SHA"
