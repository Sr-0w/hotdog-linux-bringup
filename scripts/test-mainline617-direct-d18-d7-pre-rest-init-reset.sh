...
BOOT_IMAGE="$HOTDOG_ROOT/images/pmos-experiments/2026-07-13-103400-mainline617-direct-d18-d7-pre-rest-init-reset/boot.img"
...
BOOT_SHA=f6c33388e9d4bf5589bf07ab732fe17ce44de52faf73a4aa6aa320bca0eba770
...
if [ "${1:-}" = -h ] || [ "${1:-}" = --help ]; then
	cat <<'USAGE'
Usage: test-mainline617-direct-d18-d7-pre-rest-init-reset.sh

Keep D9's complete image contract but request PSCI SYSTEM_RESET after the
complete start_kernel() sequence, immediately before rest_init(). Rollback
restores verified R6 plus the stock DTBO.
USAGE
	exit 0
fi

[ "$#" -eq 0 ] || die "This pinned D18 launcher accepts no options" 2
...
check_sha "D18 pre-rest-init reset image" "$BOOT_IMAGE" "$BOOT_SHA"
