...
BOOT_IMAGE="$HOTDOG_ROOT/images/pmos-experiments/2026-07-12-182000-mainline617-d4-primary-entry-psci-reset/boot.img"
...
BOOT_SHA=06fe64e230f3b09f693d81500bd92a207badda8309e71375f77695a95b094607
...
if [ "${1:-}" = -h ] || [ "${1:-}" = --help ]; then
  printf '%s\n' 'Usage: test-mainline617-direct-d4-entry-reset.sh'
  printf '%s\n' 'Pinned D4: no-op dtbo_b plus primary_entry PSCI reset probe.'
  exit 0
fi
[ "$#" -eq 0 ] || die "This pinned D4-entry launcher accepts no options" 2
...
check_sha "D4 entry-reset boot image" "$BOOT_IMAGE" "$BOOT_SHA"
...
exec "$HOTDOG_ROOT/scripts/test-boot-b-image.sh" \
  ...
  --restore-after system --boot-wait 180 --poll 2 --fastboot-timeout 15 \
