...
BOOT_IMAGE="$HOTDOG_ROOT/images/pmos-experiments/2026-07-13-102500-mainline617-direct-d17-d7-post-console-init-reset/boot.img"
...
BOOT_SHA=18706ac45471e88835fff678ec9d1b97d149a1a0a16da1f0818a0bff388a6e7a
...
if [ "${1:-}" = -h ] || [ "${1:-}" = --help ]; then
	cat <<'USAGE'
Usage: test-mainline617-direct-d17-d7-post-console-init-reset.sh

Keep D9's complete image contract but request PSCI SYSTEM_RESET immediately
after console_init() returns in start_kernel(). Rollback restores verified R6
plus the stock DTBO.
USAGE
	exit 0
fi

[ "$#" -eq 0 ] || die "This pinned D17 launcher accepts no options" 2
...
check_sha "D17 post-console-init reset image" "$BOOT_IMAGE" "$BOOT_SHA"
