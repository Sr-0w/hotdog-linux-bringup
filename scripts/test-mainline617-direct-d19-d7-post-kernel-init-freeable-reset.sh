...
BOOT_IMAGE="$HOTDOG_ROOT/images/pmos-experiments/2026-07-13-105300-mainline617-direct-d19-d7-post-kernel-init-freeable-reset/boot.img"
...
BOOT_SHA=d61de0104cfe226c76c5349e0af99f6cf32dff1344d575a3d92701ef44ee329d
...
if [ "${1:-}" = -h ] || [ "${1:-}" = --help ]; then
	cat <<'USAGE'
Usage: test-mainline617-direct-d19-d7-post-kernel-init-freeable-reset.sh

Keep D9's complete image contract but request PSCI SYSTEM_RESET after PID 1
returns from kernel_init_freeable(), including SMP setup, initcalls, initramfs
waiting, and rootfs and early userspace preparation. Rollback restores R6 plus
the stock DTBO.
USAGE
	exit 0
fi

[ "$#" -eq 0 ] || die "This pinned D19 launcher accepts no options" 2
...
check_sha "D19 post-kernel-init-freeable reset image" "$BOOT_IMAGE" "$BOOT_SHA"
