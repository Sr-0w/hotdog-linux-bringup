...
kernel_build="$HOTDOG_ROOT/build/clearstaff-v29-dsc-slice-per-pkt"
...
kernel_sha=3704210098904375595c7c710b321ee4923db102aa6cff8927bb86eb7963f9a4
...
build_dir="$HOTDOG_ROOT/build/experiments/$stamp-clearstaff616-v29-dsc-slice-per-pkt"
outdir="$HOTDOG_ROOT/images/pmos-experiments/$stamp-clearstaff616-direct-entry-v29-dsc-slice-per-pkt"
...
Kernel: V28 ClearStaff Linux 6.16 baseline rebuilt with upstream-style MSM DSI
        support for more than one DSC slice per packet. The OnePlus panel requests
        two 720-pixel slices in one packet, so bytes_per_pkt becomes 1440,
        pkt_per_line becomes 1, and command-mode word count becomes 1441.
...
Purpose: test the packetization mismatch identified after V27 and V28 produced
         the same stable full-screen DSI stripe pattern.
EOF
...
"$HOTDOG_ROOT/scripts/build-mainline-direct-bootimg.sh" \
	...
	--outdir "$outdir" \
	--name boot-clearstaff616-direct-entry-v29-dsc-slice-per-pkt \
...
## V29 DSC packetization contract
