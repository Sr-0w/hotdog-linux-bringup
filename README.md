...
| Persistent direct mainline | Hang isolated inside `smp_init()` | D10 through D20 prove direct boot through PID 1 and the `kthreadd` handoff. D22 reaches the boundary before `smp_init()`, while D23 never reaches the boundary after it. D24 repeats D23 with `maxcpus=1` to test secondary CPU bring-up. R6 plus stock DTBO is the verified rollback environment. |
