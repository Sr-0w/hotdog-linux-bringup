...
| Persistent direct mainline | MMU-on virtual entry proven | D10 through D13 exhausted all seven slot attempts through successively later PSCI reset checkpoints. Direct execution now reaches `__primary_switched`; D15 tests the complete assembly path immediately before `start_kernel()`. R6 plus stock DTBO is the verified rollback environment. |
