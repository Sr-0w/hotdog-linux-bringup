...
| Persistent direct mainline | `subsys`/`fs` initcalls under diagnosis | D36 proves levels 0-3 return, while D37 does not reach the checkpoint after level 5. D38 distinguishes level 4 (`subsys`) from level 5 (`fs`). |
