...
| Persistent direct mainline | Pre-MMU path proven | D10 through D12 exhausted all seven slot attempts through successively later PSCI reset checkpoints. Direct execution now reaches `__cpu_setup` immediately before `__primary_switch`; D13 tests the MMU-on virtual entry. R6 plus stock DTBO is the verified rollback environment. |
