...
| Mobile data | IPA v4.1 / rmnet | Revision `r161` adds the generation to mainline: platform data, register definitions and the device tree node. Mainline declared `IPA_VERSION_4_1` but shipped no data file and no match entry, so there was no path at all. The endpoint, resource, QSB and memory tables are derived from the published downstream kernel and re-expressed in mainline's structures. The register set is mainline's v4.2 with the five entries that differ reverted. `ipa.ko` builds and the node reaches the device tree enabled; probe, `rmnet` and ModemManager are untested. |
