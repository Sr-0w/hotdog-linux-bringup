...
| Persistent direct mainline | SMP bring-up under diagnosis | D29 forces `setup_max_cpus = 0` in-kernel and reaches the checkpoint after `bringup_nonboot_cpus()`, proving that secondary-CPU activation is the first direct-boot hang and that the earlier boot-image `maxcpus=0` tests were ineffective. D30 tests the end of `smp_init()` with the same bypass. R6 plus stock DTBO is the verified rollback environment. |
