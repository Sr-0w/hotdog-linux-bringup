...
| Audio | Digital headphone backend working; speaker silicon identified | Revision `r26` preserves the running ADSP and MPSS and opens a silent 48 kHz stereo S24_LE stream from `MultiMedia1` through `SLIMBUS_6_RX` to WCD9340 `AIF4_PB` without DSP or transport errors. Revision `r28` safely binds both TFA9874 amplifiers and reads revision `0x0c74` without writes, reset control, DAI registration, or output enable. Audible headphone output, UCM routing, earpiece, microphones, headset detection, MI2S4, ADSP speaker protection, and speaker output remain separate gates. |
