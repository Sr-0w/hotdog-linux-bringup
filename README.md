...
| Persistent direct mainline | Early `smp_init()` hang | D10 through D20 prove direct boot through PID 1 and the `kthreadd` handoff. D24 shows that `maxcpus=1` does not resolve the hang, and D25 does not reach the return from `bringup_nonboot_cpus()`. D26 tests immediately before that call. R6 plus stock DTBO is the verified rollback environment. |
