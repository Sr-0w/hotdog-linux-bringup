...
| Mainline panel | Dynamic 60/90 Hz working | Native DPU, DSI, TE, DSC, and the OnePlus Samsung panel produce correct 1440x3120 KMS scanout under `kmscube`, Weston, and Plasma Mobile. Revision `r17` switches both stock modes from KScreen and Plasma Settings with the matching panel command. A purple scanout recovered through lock/unlock; repeated blank/unblank and suspend/resume remain open. |
...
| Audio | Internal ALSA card and PCM devices working; physical routes pending | Revision `r25` preserves the running ADSP and MPSS, registers the SM8150 machine card, and exposes `MultiMedia1` playback and capture through the physical WCD9340. No speaker, earpiece, microphone, or headset route is claimed yet. UCM routing, headset thresholds, and the external TFA98xx amplifiers remain separate gates. |
