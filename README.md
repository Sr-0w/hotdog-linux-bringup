...
| Persistent direct mainline | SMP bring-up under diagnosis | D26 reaches the boundary before `bringup_nonboot_cpus()`, while D25 never reaches the boundary after it with `maxcpus=1`. D27 shows that `maxcpus=0` still does not finish all of `smp_init()`; D28 tests immediately after the skipped call. R6 plus stock DTBO is the verified rollback environment. |
