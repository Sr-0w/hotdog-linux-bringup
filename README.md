...
| Persistent direct mainline | Reaches level-6 initcalls | The guarded trace reached global initcall 524 and stopped inside `calibrate_xor_blocks()`. This is the second boot-time throughput calibration to block after RAID6 and points to non-advancing kernel time during direct boot. The next candidate disables the XOR benchmark through a default-on Kconfig switch while preserving exact per-initcall tracing. |
