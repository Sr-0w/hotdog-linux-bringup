...
| Persistent direct mainline | Single-CPU userspace test prepared | D29 and D30 prove that forcing `setup_max_cpus = 0` in-kernel carries direct boot through `bringup_nonboot_cpus()` and the complete `smp_init()`. Secondary-CPU activation is the first direct-boot hang, while boot-image `maxcpus=0` was ineffective. D31 removes the diagnostic reset and tests the real mainline userspace path. |
