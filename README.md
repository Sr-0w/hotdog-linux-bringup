...
| Persistent direct mainline | Second blocker remains | D50 isolated entry 49, `raid6_select_algo`, and disabling its benchmark let that initcall return. A full no-checkpoint candidate with the same workaround then held the fixed OnePlus logo for 480 seconds without USB or SSH. The next source-tracked diagnostic arms an entry-time watchdog, requests fastboot on reset, and records each initcall in reserved RAM; hardware validation is pending. |
