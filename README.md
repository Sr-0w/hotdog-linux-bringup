...
| Persistent direct mainline | Hang isolated inside `smp_init()` | D10 through D20 prove direct boot through PID 1 and the `kthreadd` handoff. D23 does not return from `smp_init()`, and D24 shows that `maxcpus=1` does not resolve it. D25 tests immediately after `bringup_nonboot_cpus()`. R6 plus stock DTBO is the verified rollback environment. |
