...
| GNSS | Location | Not yet hardware-validated. The userspace path needs a network port, which depends on the IPA work below. |
| Mobile data | IPA v4.1 / rmnet | Revision `r164` brings IPA v4.1 up on mainline, a generation the tree declared but never supported: platform data, register definitions and the device tree node, all derived from the published downstream kernel and re-expressed in mainline's structures. The driver binds, reports `IPA driver initialized`, and completes the modem handshake with `received modem running event`. The hardware confirms the memory map exactly, reporting 0x2800 of shared memory. No `rmnet` netdev has appeared yet, so the IPA-to-modem QMI exchange has not completed and ModemManager still finds no modem. |
