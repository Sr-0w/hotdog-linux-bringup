...
| Persistent direct mainline | Reaches general initcalls | D50 isolated entry 49, `raid6_select_algo`, and disabling its benchmark let that initcall return. An Image-resident breadcrumb has now proved that direct boot reaches `do_initcalls()` (`stage 200`). The next candidate records the exact initcall index, level, and function address before and after every call. |
