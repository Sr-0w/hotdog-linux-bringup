...
| Persistent direct mainline | Initcall boundary under diagnosis | D29 through D32 prove that the forced single-CPU path returns from `smp_init()` and `sched_init_smp()`. D31 still exposed no USB or SSH during 360 seconds. D33 tests the boundary after workqueue, async, padata, and late page-allocation setup, immediately before `do_basic_setup()`. |
