...
| Persistent direct mainline | Reaches level-6 initcalls | The guarded trace reached global initcall 524 and stopped inside `calibrate_xor_blocks()`. This is the second boot-time throughput calibration to block after RAID6 and points to non-advancing kernel time during direct boot. A first pre-MMU system-counter probe held at the fixed logo without reaching its recovery watchdog. The replacement keeps both benchmark bypasses and probes `CNTCR` only after the watchdog is armed. |
