...
| Persistent direct mainline | Early `kernel_init_freeable()` hang bounded | D10 through D20 prove direct boot through PID 1 and the `kthreadd` handoff. D21 did not reach its post-`sched_init_smp()` reset, placing the unresolved interval in early `kernel_init_freeable()`. D22 tests immediately before `smp_init()`. R6 plus stock DTBO is the verified rollback environment. |
