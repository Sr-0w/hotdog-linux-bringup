...
| Mobile data | IPA v4.1 / rmnet | Revision `r165` brings IPA v4.1 up on mainline, a generation the tree declared but never supported. Platform data, register definitions and the device tree node are derived from the published downstream kernel and re-expressed in mainline's structures; the register set is mainline's v4.2 with the five entries that differ between the generations reverted. The AP loads `ipa_fws`, merged from the stock vendor image. IPA reports setup completed, `rmnet_ipa0` exists, and ModemManager now enumerates the modem with both a QMI port and a net port, reading the IMEI from hardware. It stops at `sim-missing`, which is the correct state with no card inserted. |
