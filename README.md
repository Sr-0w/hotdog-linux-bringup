...
| Persistent direct mainline | Kernel entry proven | D10 changed only the first `primary_entry` instructions and exhausted all seven slot attempts through PSCI reset, proving direct mainline execution. D11 moves the checkpoint after initial idmap creation. R6 plus stock DTBO is the verified rollback environment. |
