...
| Persistent direct mainline | Console initialization proven | D10 through D17 exhausted all seven slot attempts through successively later PSCI reset checkpoints. Direct execution now returns from `console_init()`; D18 tests immediately before `rest_init()`. R6 plus stock DTBO is the verified rollback environment. |
