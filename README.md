...
| NFC | PN553 controller, board configuration and RF polling | The upstream NCI controller binds and `r144` applies the OxygenOS-derived RF/TVDD configuration from a packaged firmware blob. Every advertised reader protocol polls and stops cleanly. `neard` remains powered and polling, but no target object has been reported yet, so tag discovery and reading remain open. |
