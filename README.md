...
| Persistent direct mainline | `start_kernel()` proven | D10 through D18 exhausted all seven slot attempts through successively later PSCI reset checkpoints. Direct execution now completes the full `start_kernel()` sequence; D19 tests PID 1 after `kernel_init_freeable()`. R6 plus stock DTBO is the verified rollback environment. |
