...
| Persistent direct mainline | Hang in `bringup_nonboot_cpus()` | D10 through D20 prove direct boot through PID 1 and the `kthreadd` handoff. D26 reaches the boundary before `bringup_nonboot_cpus()`, while D25 never reaches the boundary after it, both with `maxcpus=1`. D27 uses `maxcpus=0` to skip the call. R6 plus stock DTBO is the verified rollback environment. |
