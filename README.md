...
| Persistent direct mainline | Architecture setup proven | D10 through D16 exhausted all seven slot attempts through successively later PSCI reset checkpoints. Direct execution now returns from `setup_arch()`; D17 tests immediately after `console_init()`. R6 plus stock DTBO is the verified rollback environment. |
