...
| Modem | MPSS remote processor / QRTR services | MPSS reaches `running`; RMTFS, QRTR, PD mapper and related services are present. With IPA up, ModemManager enumerates the modem and reads its IMEI from hardware. |
| NFC | PN553 controller, tag discovery and ISO-DEP exchange | The complete 774-byte OxygenOS configuration applies, the controller runs from its `RPMH_LN_BB_CLK3` reference clock at 19.2 MHz, and discovery is accepted for every initiator protocol. A presented ISO 14443-4 document is detected, activated, typed as tag type 4 and given a `neard` object, and neard exchanges real ISO 7816-4 APDUs with it: `SELECT by DF name` goes out and the document answers. An ePassport then refuses the read without BAC authentication, which is the correct behaviour and confirms the exchange is bidirectional. |
| Haptics | AW8697 linear resonant actuator | The source-built `FF_RUMBLE` driver drives the motor. An uploaded effect is accepted, the driver sets the chip's `GO` bit and its state machine holds a playback state for exactly the requested duration before returning to standby, with no kernel errors. Physically confirmed by hand on the handset. |
...
| Mobile data | IPA v4.1 / rmnet / RF | Revision `r165` brings IPA v4.1 up on mainline, a generation the tree declared but never supported: platform data, register definitions and the device tree node, derived from the published downstream kernel and re-expressed in mainline's structures. The AP loads `ipa_fws`, merged from the stock vendor image. `rmnet_ipa0` exists and ModemManager enumerates the modem with a QMI port and a net port, reading the IMEI from hardware. The radio is proven: driven online over QMI it scans the bands, identifies three operators by MCC/MNC across LTE and GSM, and camps on a cell, all without a SIM. Registration itself needs a card. |
...
| GNSS | Location | Not yet hardware-validated. The network port it was waiting on now exists, so the IPA dependency is cleared. |
| Sensors | Motion / rotation / proximity | Every part of the path is in place: SLPI runs from packaged firmware, `hexagonrpcd` is installed, the DSP's 17 served files are present and `/dev/fastrpc-sdsp` exists. Attaching the sensors protection domain still faults. An SMMU translation fault on the first FastRPC context bank precedes the `sensor_process` crash and lands inside the handling of the attach packet itself, so the service is reached and faults while servicing it. The reserved-memory map was checked against the stock tree and matches it exactly, so that is not the cause. The two Hall sensors are separate and already work. |
