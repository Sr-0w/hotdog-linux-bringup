...
| Persistent direct mainline | Pre-initcall blocker isolated | D50 isolated entry 49, `raid6_select_algo`, and disabling its benchmark let that initcall return. The next source-tracked candidate armed an entry-time watchdog and entered Qualcomm `900e`; its fixed breadcrumb remained at stage 1 in `primary_entry`, proving that direct boot does not reach `do_initcalls`. A hardware-pending follow-up now records MMU and `start_kernel()` milestones in an Image-resident breadcrumb. |
