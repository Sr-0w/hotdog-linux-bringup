...
| Persistent direct mainline | Post-SMP path under diagnosis | D29 and D30 prove that forcing `setup_max_cpus = 0` carries direct boot through `bringup_nonboot_cpus()` and the complete `smp_init()`. D31 continued without a reset checkpoint but exposed no USB or SSH during 360 seconds. D32 tests the boundary after `sched_init_smp()` with the same bypass. |
