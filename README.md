...
| Bluetooth | Scan, pair, disconnect and HID input | `crbtfw21.tlv` and `crnv21.bin` download at boot, `hci0` reaches `UP RUNNING`, an LE scan finds nearby devices, and an Xbox Wireless Controller pairs, trusts and connects over an authenticated encrypted link. `/dev/input/js0` delivers real axis events, and the controller reconnects by itself after `hci_uart` is reloaded. Repeated computer-to-phone pairing originally exposed upstream GENI UART bug `b93062b6d8a1`: a zero-length stale RX completion left DMA permanently disarmed, then the next HCI command crashed the Bluetooth SoC. The stable backport passes repeated bond/disconnect cycles and 102 later scan-state commands with RX still advancing. The address is locally administered but stable across boots and reloads; the factory value is provisioned per unit and is absent on this handset, exactly as for Wi-Fi. |
...
The current public image set is `v0.2.0-alpha.2`, pairing postmarketOS Plasma
Mobile with the clean Linux 6.17 kernel, matching AVB boot image, required
filtered DTBO, verification-disabled vbmeta and a rootfs installed on `userdata`.
The boot, DTBO, vbmeta, kernel APK and rootfs form one atomic set and must not be mixed across releases. The exact
