...
| Persistent direct mainline | C entry proven | D10 through D15 exhausted all seven slot attempts through successively later PSCI reset checkpoints. Direct execution now reaches the call to `start_kernel()`; D16 tests immediately after `setup_arch()`. R6 plus stock DTBO is the verified rollback environment. |
