...
| Persistent direct mainline | SMP bring-up under diagnosis | D26 reaches the boundary before `bringup_nonboot_cpus()`, while D25 never reaches the boundary after it with `maxcpus=1`. D27 and D28 show that boot-image `maxcpus=0` does not reach either later checkpoint. D29 forces the value in-kernel. R6 plus stock DTBO is the verified rollback environment. |
