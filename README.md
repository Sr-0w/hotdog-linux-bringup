...
| Persistent direct mainline | `start_kernel()` proven; first later boundary located | D10 through D18 exhausted all seven slot attempts through successively later PSCI reset checkpoints. D19 did not reach its post-`kernel_init_freeable()` reset, placing the first unresolved interval between `rest_init()` and that return. D20 tests PID 1 immediately after the `kthreadd` handoff. R6 plus stock DTBO is the verified rollback environment. |
