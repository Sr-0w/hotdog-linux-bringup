...
| Audio | Card, PCM, codec transport, and digital headphone backend working | Revision `r26` preserves the running ADSP and MPSS and opens a silent 48 kHz stereo S24_LE stream from `MultiMedia1` through `SLIMBUS_6_RX` to WCD9340 `AIF4_PB` without DSP or transport errors. Audible headphone output, UCM routing, earpiece, microphones, headset detection, and external TFA98xx speakers remain separate gates. |
