...
| Persistent direct mainline | SMP hang narrowed to two calls | D10 through D20 prove direct boot through PID 1 and the `kthreadd` handoff. D21 did not reach its post-`sched_init_smp()` reset, while D22 exhausted all retries immediately before `smp_init()`. The hang is now limited to `smp_init()` or `sched_init_smp()`; D23 tests between them. R6 plus stock DTBO is the verified rollback environment. |
