...
| NFC | PN553 controller, board configuration and RF polling | The complete 774-byte OxygenOS configuration now applies. `r160` completes the proprietary set-mode response the NCI core was dropping, keeps Geni I2C in FIFO mode so frames above 32 bytes survive the bus, and feeds the controller its `RPMH_LN_BB_CLK3` reference clock, which reports prepared and enabled at 19.2 MHz with the controller as its consumer. The adapter powers up offering Felica, MIFARE, Jewel, ISO-DEP and NFC-DEP, discovery is accepted for every initiator protocol, and neither the RF PLL nor the TX LDO reports a fault. No target has been presented yet, so tag discovery and reading remain unproven. `nfc-read` on the handset performs that test. |
