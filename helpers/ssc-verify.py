#!/usr/bin/env python3
import ctypes, importlib.util, pathlib, socket, struct, time
...
def run(name, txt, msgid, rate=None, secs=6):
    ...
    std=SSC.pb_bytes(2,SSC.field(1,5,struct.pack("<f",rate))) if rate else b""
    body=(SSC.pb_bytes(1,SSC.suid_msg(low,high))+SSC.field(2,5,struct.pack("<I",msgid))
          +SSC.pb_bytes(3,susp)+SSC.pb_bytes(4,std))
    ...
    while time.time()<end:
        ...
        while True:
            ...
            if struct.unpack_from("<I",d,i+1)[0]!=1025: continue
            n+=1
            j=d.find(b"\x0a",i)
            if 0<j<i+40 and j+2<len(d):
                ln=d[j+1]
                if 4<=ln<=48 and ln%4==0 and j+2+ln<=len(d) and first is None:
                    first=struct.unpack("<%df"%(ln//4),d[j+2:j+2+ln])
    SSC.libc.close(fd)
    print("  %-22s %4d ech.  %s"%(name,n," ".join("%9.3f"%x for x in first[:4]) if first else "-"))
run("accel  (m/s2)","1fbb6afc01727ea69a418d19f8d7ba44",513,25.0)
run("gyro   (rad/s)","97d3c48969f972a0b9418a10491511c1",513,25.0)
run("mag    (uT)","64bba517c99048b0ac450869b71d795b",513,25.0)
run("temperature (C)","973615c0c0808fa3af4002dc9cb83279",513,5.0)
run("ambient_light","5f5f5f534c4131303733534354736d61",514)
run("proximity","5f5f584f525031303733534354736d61",514)
run("motion_detect","0b68a5cde246ab85774ada686d1a4bcd",514)
run("sars","7335663959f5698867456bc70a6c70ca",514)
