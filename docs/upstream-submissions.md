...
| hci_qca: a controller that never came up | 2 | Local, compiles clean | Two independent bugs in `hci_qca`, both reached only when setup never completes. `qca_power_shutdown()` sets the baud rate and sends a power pulse through a serdev port that was never opened, and `ttyport_set_baudrate()` dereferences `serport->tty` unconditionally: removing the driver oopses in `tty_set_termios()`. `qca_suspend()` waits `FW_DOWNLOAD_TIMEOUT_MS` for a `QCA_IBS_DISABLED` that a failed setup never clears, so a dead controller aborts every system suspend on the machine at a measured 3.3 s. Neither is device-specific. Hardware-validated on r13: a full s2idle cycle completes with no `-110`, and `rmmod hci_uart` returns 0 with the boot ID unchanged. Both were exercised on a healthy controller, so what is proven is no regression plus the absence of the crash and the abort. |
| SM8150 hotdog Bluetooth UART alias | 1 | Hardware-validated locally | The board declared no `serial`/`hsuart` alias, so `qcom_geni_serial_probe()` got `-ENODEV` from `of_alias_get_id()` and logged `Invalid line -19`. No `ttyHS`, no serdev child, no Bluetooth controller at all -- which is what `hci0 DOWN` with no firmware download really meant. `hsuart0 = &uart13` matches sm8150-google-flame, -xiaomi-nabu and -xiaomi-raphael. Board-level change, goes with the device-tree series. |
| q6afe-dai: start HDMI/DisplayPort at trigger | 1 | Hardware-validated locally | `q6afe_dai_prepare()` starts the AFE port, but `snd_soc_pcm_dai_prepare()` walks CPU DAIs before codec DAIs, and for HDMI and DisplayPort the sink is hdmi-codec, which only enables the controller's audio engine from its own prepare. The port was started into a sink that was not listening and the DSP answered nothing -- `-110`, not `-EALREADY`, so `q6afe_port_start()`'s recovery never applied. Because the CPU prepare failed the codec prepare never ran either, leaving the port half-open. Configuring and starting from `trigger` gives audio on a monitor: 6/6 playbacks. Not device-specific. Two starts in six still log `-110` without stopping playback, so send it with that stated. |
