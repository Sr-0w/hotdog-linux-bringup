...
pmOS SSH comes back, this wrapper launches the prepared `050008` test with
`--from-pmos-ssh`:
...
next image: /home/srobin/dev/hotdog/images/pmos-experiments/2026-07-10-050008-lineage414-pmaports-kernel-ttyautocycle-usbifdiagwatchdog/boot-noefi-pmosdtb-watchdog-300s.img
...
The current automatic next candidate is `050008`. It supersedes `045402` while
...
gadget/UDC alone no longer stops the 300s timeout reboot. Inherited from
`045402`: the auto-cycle pages print `usb/watchdog` status with carrier,
operstate, interface address, and watchdog markers. New in `050008`: generic
`eth*` interfaces no longer count as USB watchdog success; only `usb*`,
`rndis*`, and `enx*` are considered:

```text
/home/srobin/dev/hotdog/images/pmos-experiments/2026-07-10-050008-lineage414-pmaports-kernel-ttyautocycle-usbifdiagwatchdog/boot-noefi-pmosdtb-watchdog-300s.img
sha256: b6ce8dfc5787f08792a18ec41762bf14ed3152ba1a298f871c7f38eb64a28c99
...
initramfs sha256: ea92f80d49e00ad6578fa67a790bd032a772775eb0e46c2d7bcc3fb06c931c53
...
/home/srobin/dev/hotdog/scripts/inspect-dtb-pack-simplefb.sh \
  --dtb /home/srobin/dev/hotdog/images/pmos-experiments/2026-07-10-050008-lineage414-pmaports-kernel-ttyautocycle-usbifdiagwatchdog/components/dtb \
  --entry 12
```

The verified `050008` entry 12 result is:
...
`050008` tests the pmaports kernel config required for fbcon with the most useful
...
3. Test the prepared downstream `050008` image once the phone is back in a
   ...
   `045402`, it keeps automatic screen page rotation every 12s, carrier/up
   USB watchdog success, and visible `usb/watchdog` status, then narrows USB
   success to `usb*`, `rndis*`, and `enx*` so a generic `eth*` interface cannot
   stop the rescue reboot.
...
4. The fixed entry12 `ranges;` DTB pack has already been promoted into the
   ...
   `pmbootstrap checksum linux-oneplus-hotdog-lineage414`. If `050008` still
   reports `No memory resource`, continue in the simplefb resource translation
   path rather than changing printk cmdline flags. If `050008` fails earlier
...
6. Run `scripts/validate-current-candidates.sh` before the next hardware cycle
   ...
   It validates `050008`, `034200`, and `025400` without phone commands.
