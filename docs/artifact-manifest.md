...
| `/home/srobin/dev/hotdog/scripts/build-entry12-simplefb-ranges-dtbpack.sh` | Rebuilds the fixed entry12 simplefb `ranges;` DTB pack used by the prepared `014400` image. |
...
| `/home/srobin/dev/hotdog/scripts/test-next-lineage414-simplefb-shell.sh` | Wrapper for the next `014400` hardware test. It restores `boot_b` to `215005` and starts a companion rescue watcher. |
| `/home/srobin/dev/hotdog/scripts/wait-pmos-then-test-next-lineage414-simplefb-shell.sh` | Waits for pmOS SSH, then launches the `014400` test from the booted pmOS system with `--from-pmos-ssh`. |
