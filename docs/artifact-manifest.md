...
| `/home/srobin/dev/hotdog/images/pmos-experiments/2026-07-10-044715-lineage414-pmaports-kernel-ttyautocycle-usbcarrierwatchdog/boot-noefi-pmosdtb-watchdog-300s.img` | Current next downstream 4.14 pmaports-kernel screen candidate. It keeps the visible tty auto-cycle from `045000`, but tightens USB watchdog success to a carrier/up USB network interface instead of a configured gadget/UDC. This should avoid staying stuck in a rootfs state that has screen text but no host-visible USB. |
| `/home/srobin/dev/hotdog/images/pmos-experiments/2026-07-10-045000-lineage414-pmaports-kernel-ttyautocycle-strictusbwatchdog/boot-noefi-pmosdtb-watchdog-300s.img` | Superseded visible-tty auto-cycle candidate. It keeps the `042500` strict USB-success watchdog and makes the rootfs visible tty shell auto-cycle full status, display/network, and dmesg pages every 12s when there is no SSH or button input. Superseded by `044715`, which requires USB carrier/up state before stopping the watchdog. |
| `/home/srobin/dev/hotdog/images/pmos-experiments/2026-07-10-042500-lineage414-pmaports-kernel-ttykmsg-buttonshell-strictusbwatchdog/boot-noefi-pmosdtb-watchdog-300s.img` | Superseded strict USB-watchdog downstream screen candidate. It keeps the `034500` kernel/DTB/tty/button diagnostics, but uses a strict USB-success watchdog: root mount/switch-root alone no longer disables the watchdog, and timeout tries sysrq reboot before `reboot -f`. Superseded by `045000`, which adds automatic visible tty page rotation. |
| `/home/srobin/dev/hotdog/images/pmos-experiments/2026-07-10-034500-lineage414-pmaports-kernel-ttykmsg-buttonshell-rootwatchdog/boot-noefi-pmosdtb-watchdog-300s.img` | Superseded observation-friendly downstream screen candidate. It keeps the `033600` kernel/DTB path, raises `loglevel=8`, keeps all `fbcon=` args, and adds a rootfs visible tty button console: Vol+ full status, Vol- network/display status, Power dmesg tail. Superseded as automatic next by `042500`, `045000`, then `044715`. |
| `/home/srobin/dev/hotdog/images/pmos-experiments/2026-07-10-034200-lineage414-pmaports-kernel-splash-ttykmsg-fbprep-rootwatchdog/boot-noefi-pmosdtb-watchdog-300s.img` | Secondary downstream 4.14 screen candidate. It keeps the `033600` hooks and additionally appends `splash` so pmOS runs its normal initramfs `setup_framebuffer()` path before starting Plymouth/splash. Use after `044715`/`045000` if fb0/fbcon still looks timing-gated. |
| `/home/srobin/dev/hotdog/images/pmos-experiments/2026-07-10-033600-lineage414-pmaports-kernel-ttykmsg-fbprep-screen-shell-rootwatchdog/boot-noefi-pmosdtb-watchdog-300s.img` | Superseded downstream 4.14 pmaports-kernel console candidate. It keeps the `030600` kernel/DTB/cmdline shape, adds an initramfs `/dev/tty0` kmsg/status follower before the DRM helper, waits for `/dev/fb0`, sets an empty framebuffer mode when possible, boosts backlight, and installs the rootfs `hotdog-visible-tty-shell` local.d hook after `switch_root`. Superseded by `034500`, `042500`, `045000`, then `044715`. |
| `/home/srobin/dev/hotdog/images/pmos-experiments/2026-07-10-030600-lineage414-pmaports-kernel-screen-shell-rootwatchdog/boot-noefi-pmosdtb-watchdog-300s.img` | Superseded downstream 4.14 pmaports-kernel console candidate. It keeps the `024200` kernel/DTB/cmdline shape, keeps DRM helper Vol+/Vol- diagnostics, and adds a rootfs `hotdog-visible-tty-shell` local.d hook so a tty1/tty0 shell prompt plus status follower appears on screen after `switch_root`. Superseded by `033600`, `034500`, `042500`, `045000`, then `044715`. |
...
| `/home/srobin/dev/hotdog/scripts/build-entry12-simplefb-ranges-dtbpack.sh` | Rebuilds the fixed entry12 simplefb `ranges;` DTB pack used by the prepared `014400`, `015500`, `024200`, `030600`, `033600`, `034200`, `034500`, `042500`, `045000`, and `044715` images. |
...
| `/home/srobin/dev/hotdog/scripts/validate-current-candidates.sh` | Offline validation gate for the current next and secondary boot candidates. It checks wrapper paths, expected cmdline words, entry12 simplefb wiring, generated helper shell syntax, and the current next strict USB-watchdog semantics without using phone commands. |
...
| `/home/srobin/dev/hotdog/scripts/test-next-lineage414-simplefb-shell.sh` | Wrapper for the next `044715` hardware test. It restores `boot_b` to `215005` and starts a companion rescue watcher. |
...
| `/home/srobin/dev/hotdog/reports/lineage414-openela-diff-20260709-140656/91-strict-usb-watchdog-candidate-20260710.txt` | Prepared boot image `042500`, which promotes the `034500` screen/button diagnostics to the automatic next test with strict USB-success watchdog semantics and sysrq-first timeout reboot. |
| `/home/srobin/dev/hotdog/reports/lineage414-openela-diff-20260709-140656/93-visible-tty-autocycle-candidate-20260710.txt` | Prepared boot image `045000`, which promotes `042500` to the automatic next test and adds auto-cycling visible tty status pages after switch_root. |
| `/home/srobin/dev/hotdog/reports/lineage414-openela-diff-20260709-140656/94-usb-carrier-watchdog-candidate-20260710.txt` | Prepared boot image `044715`, which promotes `045000` to the automatic next test and requires USB network carrier/up state before stopping the watchdog. |
