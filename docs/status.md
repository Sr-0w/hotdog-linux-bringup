...
| Apps SMMU | DWC3 validated; UFS translated-domain boot validated | V43 attaches DWC3 to stream ID `0x140` and uses a translated domain successfully with generic IOMMU and DWC3 source. Revision `r18` boots UFS on stream `0x300` with ICE disabled, but its DMA64 path still enters `900e` under a large buffered Flatpak import. Revision `r19` keeps translation and restores the 32-bit UFS DMA aperture; hardware validation is pending. Other clients and the complete SoC description remain unvalidated. |
