...
| Persistent direct boot | Full `start_kernel()` proven; later failure interval located | D18 exhausted all slot attempts before `rest_init()`. D19 did not reach its post-`kernel_init_freeable()` reset, placing the first unresolved interval between those boundaries. D20 tests immediately after the PID 1/`kthreadd` handoff. |
...
18. Test D20 after PID 1 observes `kthreadd` completion to separate task and
   scheduler handoff from SMP setup and initcalls.
19. Defer the r4 package-generated direct image until a direct handoff baseline
   works. Record its kernel, installed DTB, raw-image, and AVB hashes without
   reusing the historical r0 identity.
20. After a direct mainline entry succeeds, test the hotdog-only PON reboot-mode
