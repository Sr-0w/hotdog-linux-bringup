...
| Persistent direct boot | PID 1 and `kthreadd` handoff proven; init body pending | D18 proved the complete `start_kernel()` sequence. D19 did not reach the end of `kernel_init_freeable()`, while D20 exhausted all attempts after the PID 1/`kthreadd` handoff. D21 tests after SMP initialization. |
...
19. Test D21 after `sched_init_smp()` to separate SMP bring-up from topology
   setup and regular initcalls.
20. Defer the r4 package-generated direct image until a direct handoff baseline
   works. Record its kernel, installed DTB, raw-image, and AVB hashes without
   reusing the historical r0 identity.
21. After a direct mainline entry succeeds, test the hotdog-only PON reboot-mode
