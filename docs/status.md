...
| UFS | Direct boot and large buffered imports working | Raw 1 GiB I/O and 120 seconds of direct random I/O pass. The buffered OSTree object entered Qualcomm `900e` through `r21` because Linux allocated stock-owned memory omitted from the DT. With the complete `r22` reservation union, the same 0 A.D. pull wrote 5,512,437,760 block-layer bytes, deployment completed, and the installed game ran without UFS, ext4, panic, oops, or Qualcomm-transition evidence. |
...
can therefore proceed over the publication-shaped system:
