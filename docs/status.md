...
| A/B success and reboot | Working | `qbootctl` marks the active slot successful; six clean software reboot cycles returned to pmOS. `RESTART2("bootloader")` now reaches protocol-valid bootloader fastboot remotely and `fastboot reboot` returns to pmOS. The historical image-pipeline retry regression and direct recovery-mode validation still need completion. See [A/B retry regression](evidence/2026-08-17-ab-slot-retry-regression.md) and [ICE/reboot-mode validation](evidence/2026-08-25-ufs-ice-reboot-mode.md). |
...
| USB device / recovery | Partial | NCM networking and SSH are stable. ACM provides a packaged OpenRC-managed root console and survives full function recreation. Remote bootloader entry is now validated through `RESTART2`; recovery selection and the final non-laboratory recovery/rollback flow remain. |
