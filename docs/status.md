...
| Persistent direct boot | Initcall boundary under diagnosis | D29 through D32 prove that the forced single-CPU path returns from `smp_init()` and `sched_init_smp()`. D31 still exposed no USB or SSH during 360 seconds. D33 tests the boundary immediately before `do_basic_setup()`. |
...
31. Test D33 after workqueue topology, async, padata, and late page-allocation
   setup, immediately before `do_basic_setup()`. A reset loop isolates the next
   unresolved interval to the general initcall path or later userspace setup.
32. Defer the r4 package-generated direct image until a direct handoff baseline
   works. Record its kernel, installed DTB, raw-image, and AVB hashes without
   reusing the historical r0 identity.
33. After a direct mainline entry succeeds, test the hotdog-only PON reboot-mode
