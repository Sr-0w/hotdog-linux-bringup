...
| Proximity | Partial | The passive TCS3701 channel heuristic was disproved by an uncovered dark-state false `near`; OxygenOS instead uses the ADSP Elliptic ultrasonic path. That path now reports: sending the microphone index and suspend values OxygenOS writes before enabling the engine produced sensorhub parameter-id 16, which had never arrived. A recorded two-minute run gave 5 complete near/far cycles, strictly alternating, no missed state and no flapping, with far following each removal within seconds. It is a classifier for a head against the earpiece, not a rangefinder, and deliberately ignores a flat cardboard box. An IIO proximity device is now registered, `elliptic_proximity`, with `in_proximity_raw`, threshold events in both directions, `-EAGAIN` rather than a fabricated `far` before the engine speaks, and a udev rule stating `PROXIMITY_NEAR_LEVEL=1`. Remaining: the device is unused. `iio-sensor-proxy` here is linked against libssc and reports the SEE TCS3701 proximity sub-sensor, which never emits, so `ProximityNear` stays false and the screen does not blank during a call. See [near and far](evidence/2026-08-25-proximity-reports-near-and-far.md), [the protocol](evidence/2026-08-25-elliptic-protocol-from-oxygenos.md) and [the port](evidence/2026-08-24-elliptic-ultrasonic-proximity-port.md). |
