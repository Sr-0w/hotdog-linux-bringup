...
| Cameras | Not started; large but tractable | There is still no `qcom,sm8150-camss`, but linux-next carries `qcom,sm6150-camss`, the same Titan generation, which is a usable template. Of the four sensors identified from the stock vendor modules, Samsung S5K3M5 already has an upstream driver in linux-next; Sony IMX586, IMX481 and IMX471 do not. The OnePlus downstream kernel is available locally and carries the register sequences under `cam_sensor_module`, so the sensor drivers are writable. The OxygenOS userspace itself is proprietary and its `cam_req_mgr` architecture does not map onto CAMSS, so this is knowledge reuse rather than code reuse. See the [feasibility survey](evidence/2026-08-07-mainline616-remaining-hardware.md). |
