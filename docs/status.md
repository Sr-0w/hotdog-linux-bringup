...
| Persistent direct boot | Reaches level-6 initcalls; system-counter test prepared | A guarded Image-resident trace reached global initcall 524, level 6, and stopped inside `calibrate_xor_blocks()`. Its `do_xor_speed()` waits for `ktime_get()` to advance, matching the earlier RAID6 timing-loop failure. The next verified image keeps both benchmark bypasses, explicitly enables SM8150 `CNTCR.EN`, and records raw `CNTCR`/`CNTVCT_EL0` samples for Sahara. This is a testable hypothesis, not yet a validated fix. |
