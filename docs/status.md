...
| Cameras | SoC side written and building; no sensor yet | Revision `r51` adds one. `qcom,sm8150-camss` now exists in `camss.c` with four CSIPHYs, two CSIDs and two VFEs, modelled on SDM845, which is the same Titan generation and is already in this tree; and `sm8150.dtsi` gained the matching node with its register bases, interrupts, 31 clocks, power domains and SMMU streams, taken from the published OnePlus downstream device tree. `qcom-camss.ko` builds and the node is in the device tree, left disabled until a board describes sensors. Nothing has been streamed yet. Of the four sensors, Samsung S5K3M5 has an upstream driver in linux-next and is the intended first target; Sony IMX586, IMX481 and IMX471 still need drivers, for which the downstream register sequences are available. See the [camera port plan](camera-port-plan.md). |
