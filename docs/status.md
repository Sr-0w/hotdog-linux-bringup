...
| Modem / telephony | Partial | MPSS, RMTFS, QRTR, PD mapper and QMI services run. QMI and ModemManager expose both physical SIM slots; a boot-ordered UIM helper selects the present slot 2, and a ModemManager fix routes PIN operations to that slot instead of hard-coded slot 1. The correct PIN was accepted without consuming retries, after which the previously packaged OOS12 MPSS watchdoged in `RFLM@qsf_hl_seq.c`. The matching OOS10 MPSS is now installed, QMI-confirmed and stable without a SIM for 120 seconds; registration, LTE data, SMS, calls and IMS await the guarded SIM retry. See [dual-SIM slot 2 and PIN routing](evidence/2026-08-24-dual-sim-slot2-pin.md). |
