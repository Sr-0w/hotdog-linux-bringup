...
| Audio | Digital wired-headphone backend working; analog endpoints pending | Revision `r26` keeps ADSP and MPSS in `running`, exposes card 0 `OnePlus 7T Pro`, and opens a silent 48 kHz stereo S24_LE stream from `MultiMedia1` through `SLIMBUS_6_RX` to WCD9340 `AIF4_PB`. The stream completes without DSP or transport errors and cleanup closes the PCM and mixer route. UCM, safe analog codec controls, headset thresholds, TFA98xx amplifiers, and physical headphone, speaker, earpiece, and microphone tests remain open. |
