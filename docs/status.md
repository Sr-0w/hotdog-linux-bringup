...
| SLPI infrastructure | Partial | SLPI boot, FastRPC, writable Hexagon service, registry regeneration, QRTR, SSC requests and ULog forensics work end to end. The served file tree is now complete: socinfo, the OnePlus `project_name` and `oppoVersion` entries, and the numbered registry files pre-created to sidestep the missing `O_CREAT`. Only infrastructure SUIDs are published. |
| Motion/light sensors | Broken | LSM6DSM, MMC5603x and TCS3701 publish no physical SUID, but the cause moved on 2026-08-19. The sensor PD is created by the host FastRPC attach, not by booting the SLPI, and neither `hexagonrpcd` nor the tree it serves was present on this rootfs: every earlier reading was taken against a sensor core that had never been instantiated. With the tree rebuilt from the handset's own `persist` partition and the OxygenOS vendor image, the DSP completes 3943 file operations with zero failures and registers `service 400`. A fresh SLPI coredump then shows the `ICBARB_ERROR_NO_ROUTE_TO_SLAVE` rejection **no longer occurs**: `ICB Arb Log` holds only granted bandwidth. The live blocker is the SSC serial engine, `spi_power_on : invalid param` and 41 occurrences of `spi_power_off : clock disable failed`. The downstream 4.14 control is ruled out rather than pending: that kernel carries only the downstream `adsprpc` interface, so `hexagonrpcd` cannot attach and its arm would have no sensor core for reasons unrelated to the SSC. |
