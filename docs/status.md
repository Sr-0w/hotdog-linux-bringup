...
| Bluetooth | Partial | The controller initialises from a cold boot on `r13`: `crbtfw21.tlv` and `crnv21.bin` download, `QCA setup on UART is completed`, `hci0` is `UP RUNNING` with 0 errors over 3021 commands. The root cause was never a driver or firmware fault -- the board declared no `hsuart` alias, so `of_alias_get_id()` returned `-ENODEV`, `serial@c8c000` never probed, and no serdev child existed for `hci_uart` to bind. Two `hci_qca` bugs found on the way are also fixed: suspend no longer aborts (`-110` gone) and `rmmod hci_uart` no longer oopses in `tty_set_termios()`. Module reload does not restore `hci0`, scanning and pairing are unvalidated, and the address is still locally administered. See [Bluetooth](evidence/2026-08-26-bluetooth-two-bugs-behind-one-dead-controller.md). |
