...
| Persistent direct boot | Reaches general initcalls; exact blocker under instrumentation | D42 held the fixed logo at entry 49 after D43-D50 proved entries through 48. Disabling `CONFIG_RAID6_PQ_BENCHMARK` let `raid6_select_algo` return. A guarded Image-resident breadcrumb subsequently reached `stage 200`, proving that direct boot enters `do_initcalls()`. The next image records the exact initcall index, level, and function address around every call. |
