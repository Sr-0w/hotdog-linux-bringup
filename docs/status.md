...
| Persistent direct boot | Hang isolated inside `smp_init()` | D20 proved PID 1 and the `kthreadd` handoff. D23 did not return from `smp_init()`, and D24 shows that limiting activation to one CPU does not resolve the hang. D25 tests immediately after `bringup_nonboot_cpus()`. |
...
23. Test D25 immediately after `bringup_nonboot_cpus()` with `maxcpus=1`. A
   reset loop isolates the remaining hang to the final architecture SMP setup;
   a fixed logo places it in the earlier idle/hotplug preparation interval.
24. Defer the r4 package-generated direct image until a direct handoff baseline
   works. Record its kernel, installed DTB, raw-image, and AVB hashes without
   reusing the historical r0 identity.
25. After a direct mainline entry succeeds, test the hotdog-only PON reboot-mode
