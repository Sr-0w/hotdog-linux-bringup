...
| Cameras | Telephoto and CSID TPG stream; CAMNOC rejects VFE DMA | Revisions `r51` through `r82` add SM8150 CAMSS/CCI, board wiring, the Samsung S5K3M5 driver, correct CSIPHY 1.1 programming and diagnostics. The telephoto emits complete RAW10 frames at 30 fps. The independent CSID generator also produces frame interrupts with the physical sensor and CSIPHY disconnected, but every userspace buffer retains 100 percent of its `0xAA` prefill. This rules out the sensor, CCI, MIPI link and CSIPHY. A read-only hardware register capture found a latched CAMNOC `Disconnected target` write error at the exact VFE buffer IOVA, `0x7ff80000`; the Apps SMMU stream match, translation context and global fault registers are valid. Revision `r82` proved that adding the separate internal CAMNOC vote is a no-op; both paths share RPMh BCM `MM1`. The stronger remaining omission is `GCC_CAMERA_HF_AXI_CLK`: the port declares it, downstream CPAS enables it, but CAMSS never requests it. Revision `r83` is the built, hardware-pending single-clock candidate. The three Sony sensors remain unimplemented; exploratory PM8009 rail enablement caused PMIC-level resets and is paused. See the [camera port plan](camera-port-plan.md). |
