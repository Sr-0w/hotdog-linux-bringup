...
| Modem / telephony | Partial | MPSS, RMTFS, QRTR, PD mapper and QMI services run. QMI exposes both physical SIM slots, joins each application to its physical ICCID, and the PIN path is slot-scoped. The correct PIN was accepted without consuming retries, but DMS was exposed before OxygenOS's PDC/MCFG bootstrap and the modem restarted. The matching OOS10 MPSS and its 69-profile MCFG catalog are installed; the read-only daemon confirms PDC is `NotProvisioned` and fails closed without a card. ModemManager is removed from both runlevels and D-Bus auto-activation is disabled so Plasma cannot race the pre-online owner. Gated load/select/activate/verify, readiness handoff, registration, LTE data, SMS, calls and IMS remain. See [dual-SIM slot 2 and PIN routing](evidence/2026-08-24-dual-sim-slot2-pin.md), [the MCFG catalog reconstruction](evidence/2026-08-25-oxygenos-mcfg-catalog.md) and [the ModemManager gate](evidence/2026-08-25-modemmanager-preonline-gate.md). |
