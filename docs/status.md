...
| Cameras | Telephoto RAW10 capture and manual focus working; remaining sensors and libcamera open | Revisions `r51` through `r83` add SM8150 CAMSS/CCI, board wiring, the Samsung S5K3M5 driver, CSIPHY 1.1 support and the missing HF AXI bridge. Clean-boot captures produce complete 16,423,680-byte 4208×3120 RAW10 buffers with distinct hashes and no CAMNOC or SMMU error. Revisions `r84` through `r87` identify this handset's telephoto actuator as a Semco LC898217XC at CCI address `0x74`, add a V4L2 lens driver, and calibrate `focus_absolute` to the vendor module range 0–400. A cold `r87` boot binds the actuator directly; matched captures at positions 0 and 400 show a clearly blurred and clearly focused ceiling respectively. Automatic focus/exposure/gain, color tuning, libcamera, IMX586/IMX481/IMX471 and camera-app integration remain open. See the [capture evidence](evidence/2026-08-09-mainline616-camera-telephoto.md), [focus evidence](evidence/2026-08-09-mainline616-camera-telephoto-focus.md) and [port plan](camera-port-plan.md). |
