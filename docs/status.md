...
| Audio | Digital headphone backend working; both speaker amplifiers identified safely | Revision `r26` keeps ADSP and MPSS in `running`, exposes card 0 `OnePlus 7T Pro`, and opens a silent 48 kHz stereo S24_LE stream from `MultiMedia1` through `SLIMBUS_6_RX` to WCD9340 `AIF4_PB`. Revision `r28` binds both TFA9874 amplifiers and reads the exact `0x0c74` revision without writes, reset control, DAI, or output enable. UCM, safe analog codec controls, headset thresholds, MI2S4, ADSP speaker protection, and physical headphone, speaker, earpiece, and microphone tests remain open. |
