...
| UFS ICE | Working | The recorded failure was stale: on the running device `1d90000.crypto` binds to `qcom-ice` and `gcc_ufs_phy_ice_core_clk` runs at 300 MHz. Only the link was missing -- upstream `sm8150.dtsi` carries `qcom,ice = <&ice>` and the board deleted the property. Restoring it, UFS probes, the rootfs mounts and `/sys/block/sda/queue/crypto` exposes the profile with `modes`, `num_keyslots`, `max_dun_bits` and `raw_keys`. Proven on a temporary `fastboot boot`, not yet flashed. |
