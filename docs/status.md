...
| SLPI sensor DSP | Partial | The sensors stream and SensorProxy answers, but the user protection domain is killed by its watchdog every 40.17 s: `qmi_decode_string_elem: String len 128 >= Max Len 65`, then `USER-PD DOG`, then recovery in ~150 ms. `state` reads `running` at any sampled moment, so the runtime gate passes 35/35 through it. The 65 comes from `SERVREG_NAME_LENGTH + 1` in the PDR definitions, and the over-long path is not in anything this port serves. With a dock attached the boot stalls on it. See [SLPI watchdog](evidence/2026-08-27-slpi-watchdog-every-40-seconds.md). |
