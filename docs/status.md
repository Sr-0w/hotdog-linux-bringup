...
| Persistent direct boot | Early `smp_init()` hang | D20 proved PID 1 and the `kthreadd` handoff. D24 shows that limiting activation to one CPU does not resolve the hang, and D25 did not reach the checkpoint after `bringup_nonboot_cpus()`. D26 tests immediately before that call. |
...
24. Test D26 after idle/hotplug thread setup and immediately before
   `bringup_nonboot_cpus()`. A reset loop identifies that call as the blocking
   interval; a fixed logo moves the hang into the two preceding setup calls.
25. Defer the r4 package-generated direct image until a direct handoff baseline
   works. Record its kernel, installed DTB, raw-image, and AVB hashes without
   reusing the historical r0 identity.
26. After a direct mainline entry succeeds, test the hotdog-only PON reboot-mode
