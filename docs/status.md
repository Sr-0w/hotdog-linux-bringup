...
| Modem / telephony | Partial | MPSS, RMTFS, QRTR, PD mapper and QMI services run. QMI and ModemManager expose both physical SIM slots; a boot-ordered UIM helper selects the populated slot, and a ModemManager fix routes PIN operations to that slot instead of hard-coded slot 1. The correct PIN was accepted without consuming retries, but DMS was exposed before OxygenOS's PDC/MCFG bootstrap and the modem restarted. The matching OOS10 MPSS is stable without a SIM, its 69-profile MCFG catalog is now reproducibly packaged, and the read-only daemon confirms the current PDC state is `NotProvisioned`. Gated load/select/activate/verify, registration, LTE data, SMS, calls and IMS remain. See [dual-SIM slot 2 and PIN routing](evidence/2026-08-24-dual-sim-slot2-pin.md) and [the MCFG catalog reconstruction](evidence/2026-08-25-oxygenos-mcfg-catalog.md). |
