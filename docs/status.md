...
| SLPI infrastructure | Partial | SLPI boot, FastRPC, writable Hexagon service, registry regeneration, QRTR, SSC requests, standard event subscriptions and ULog forensics work end to end. The served file tree includes socinfo, the OnePlus `project_name` and `oppoVersion` entries, and pre-created numbered registry files to sidestep the missing `O_CREAT`. A curated board config now publishes the physical SX9324 SAR SUID and real on-change events. |
| Motion/light sensors | Partial | SX9324 SAR is hardware-validated through a real SSC `sars` event stream. LSM6DSM, MMC5603x and TCS3701/ALSPS still publish no SUID. LSM6DSM is blocked by `spi_plat_init: npa_create_sync_client_ex failed`; ALSPS votes both rails but never submits an I2C transaction, even when SAR is removed, and MMC5603x likewise never opens its port. The next boundary is sensor-PD diag/address-map analysis, not further blind config permutations. See [the curated sensor proof](evidence/2026-08-20-curated-sensor-config-and-sar-events.md). |
