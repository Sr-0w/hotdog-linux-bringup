...
| Audio | Internal speakers working through Plasma | Revision `r32` matches the OxygenOS QUAT MI2S S24_LE format, 3.072 MHz BCLK, and TFA9874 slot profile. Independent channel captures validate the upper slot-0 and lower slot-1 speakers. Device package `3-r8` supplies a minimal speaker-only UCM profile, and a regular PulseAudio playback produces an independently measured approximately -46.8 dBFS 1 kHz component. Sink suspension closes the PCM and powers down both codecs; Plasma volume changes are software-only. Revision `r35` completes the capture transport: a second `MultiMedia2` front end gives capture its own PCM, and mapping the ports of every SLIMbus link rather than only the first moved capture from exact digital silence to live data. The internal microphone is plumbed and exposed as a PulseAudio source but is not acoustically validated: a 1 kHz reference tone stayed indistinguishable from the codec noise floor, and the bias permutation is the leading suspect. Protection telemetry, conservative default volume, headphones, earpiece, working microphones, and headset detection remain open. |
