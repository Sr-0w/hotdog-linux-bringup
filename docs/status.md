...
| UFS | Direct boot working; buffered-import stability under investigation | V33 enumerates the Samsung UFS with a conditional 32-bit DMA mask while Apps SMMU is bypassed. Raw 1 GiB I/O and a 120-second direct random-I/O test pass with runtime power forced on. A pull-only Flatpak test reproducibly enters Qualcomm `900e` during a multi-gigabyte buffered OSTree import, without a Linux panic or UFS/ext4 error. Two strict `r18` builds restore only UFS Apps SMMU stream `0x300` for the next hardware A/B. |
...
| Apps SMMU | DWC3 validated; UFS candidate prepared | V43 attaches DWC3 to stream ID `0x140` and uses a translated domain successfully with generic IOMMU and DWC3 source. Revision `r18` restores UFS stream `0x300` while keeping ICE disabled; hardware validation is pending. Other clients and the complete SoC description remain unvalidated. |
