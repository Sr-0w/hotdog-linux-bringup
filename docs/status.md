...
| USB device / recovery | Partial | NCM networking and SSH are stable. ACM provides a packaged OpenRC-managed root console and survives full function recreation. Remote bootloader and recovery selection are validated through `RESTART2`; the existing recovery exposes authorized root ADB. A native pmOS recovery and final installer rollback flow remain. |
