...
| Persistent direct boot | Post-SMP path under diagnosis | D29 and D30 prove that forcing `setup_max_cpus = 0` carries direct boot through `bringup_nonboot_cpus()` and all of `smp_init()`. D31 continued without a reset checkpoint but exposed no USB or SSH during 360 seconds. D32 tests the boundary after `sched_init_smp()` with the same bypass. |
...
30. Test D32 with the forced bypass and a checkpoint immediately after
   `sched_init_smp()`. A reset loop moves the next boundary into later
   `kernel_init_freeable()` initialization.
31. Defer the r4 package-generated direct image until a direct handoff baseline
   works. Record its kernel, installed DTB, raw-image, and AVB hashes without
   reusing the historical r0 identity.
32. After a direct mainline entry succeeds, test the hotdog-only PON reboot-mode
