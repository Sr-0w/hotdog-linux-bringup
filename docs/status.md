...
| USB ACM serial | Working | The OpenRC-managed `ttyGS0` console provides a real bidirectional root session at a stable host by-id path. Removing the ACM function entirely and restarting the service recreates it; ACM and NCM both return, with NCM ping passing without reboot. |
...
| Proximity | Partial | The passive TCS3701 channel heuristic was disproved by an uncovered dark-state false `near`; OxygenOS instead uses the ADSP Elliptic ultrasonic path. That path now reports: sending the microphone index and suspend values OxygenOS writes before enabling the engine produced sensorhub parameter-id 16, which had never arrived. A recorded two-minute run gave 5 complete near/far cycles, strictly alternating, no missed state and no flapping, with far following each removal within seconds. It is a classifier for a head against the earpiece, not a rangefinder, and deliberately ignores a flat cardboard box. The full chain now works and is measured: engine to IIO to `net.hadess.SensorProxy`, 29 of 29 `ProximityNear` transitions matched to an engine transition, median latency 0.72 s against the driver's 700 ms poll. That needed the channel to report 2 rather than 1, since `iio-poll-proximity` scales its near level by 1.1 and a binary channel can never exceed its own threshold. The path is now armed on demand: the driver reports when `in_proximity_raw` is being read, and `hotdog-proximity-arm` raises and drops the audio path around that, measured armed within 3 s of a claim and released 5 s after it ends. Remaining: `iio-sensor-proxy` segfaults on `SIGTERM` in the local aport's SSC shutdown path, so every stop or restart crashes. See [near and far](evidence/2026-08-25-proximity-reports-near-and-far.md), [the protocol](evidence/2026-08-25-elliptic-protocol-from-oxygenos.md) and [the port](evidence/2026-08-24-elliptic-ultrasonic-proximity-port.md). |
