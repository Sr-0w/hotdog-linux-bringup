...
| Persistent direct boot | Pre-initcall blocker under instrumentation | D42 held the fixed logo at entry 49 after D43-D50 proved entries through 48. Disabling `CONFIG_RAID6_PQ_BENCHMARK` let `raid6_select_algo` return. A later direct candidate entered Qualcomm `900e` with only the `primary_entry` breadcrumb written, so it did not reach `do_initcalls`. The next hardware-pending image records MMU and `start_kernel()` boundaries in an Image-resident breadcrumb. |
