...
| Bluetooth | Working | Scan, pair, HID input and ordinary disconnects are validated. `crbtfw21.tlv` and `crnv21.bin` download, `hci0` is `UP RUNNING`, and an Xbox controller reaches an authenticated encrypted link and delivers real input events. Computer-to-phone pairing then reproduced a separate failure: a zero-length GENI RX DMA completion permanently disarmed receive, so Write Scan Enable timed out and deliberately crashed the Bluetooth SoC. Stable commit `b93062b6d8a1` keeps the zero-length path armed. r30 passes six complete pairings, five remove/re-pair cycles and 102 later scan-state commands over five minutes with RX advancing, one boot ID and no timeout, dump or QCA reset. Reloading `hci_uart` also recovers the controller without reboot. The address remains locally administered but stable. See [controller bring-up](evidence/2026-08-26-bluetooth-two-bugs-behind-one-dead-controller.md) and [RX DMA](evidence/2026-08-28-bluetooth-zero-rx-dma.md). |
| Audio | Partial | Both internal speakers and the handset microphone work through packaged UCM, restored on 6.17 after the base tree's new tfa9872 driver claimed `nxp,tfa9874` and displaced the validated tfa9874 driver, overwriting its own DAPM widgets and silencing both amplifiers. DisplayPort audio works: the AFE port was started from the CPU DAI prepare, before the codec prepare enables the controller's audio engine, so the DSP never answered; configuring and starting from `trigger` fixes it, and a UCM `HDMI` device gives Plasma the sink. The audio backend was also `postmarketos-base-ui-audio-backend-pulseaudio`, whose daemon held `/dev/snd` and left WirePlumber with no devices at all -- that, not 6.17, is why Plasma showed neither output nor input. Earpiece, remaining microphones, headset/USB-C detection, Bluetooth/call audio, capture controls and protection telemetry remain. See [DisplayPort audio](evidence/2026-08-27-displayport-audio-the-sink-was-started-before-it-listened.md). |
| USB-C host / dock | Partial | The 6.16 oracle validates dual-role Type-C, powered host/sink and unpowered host/source modes, USB 3, storage, Ethernet and DisplayPort video at 2560x1440@60 in both orientations, followed by gadget/SSH recovery. The current 6.17 r30 image regressed dock insertion: a complete no-reset 900e RAM capture ends immediately after the GenesysLogic USB3 and USB2 hubs enumerate, before child devices, DP HPD or audio, with no Linux panic. Current work is isolating the new DP jack-presence consumer from the 6.17 host/Type-C path. DP mode pruning remains correct: 2560x1440@120 exceeds the two-lane HBR2 budget and is refused. See [dock 900e ramdump](evidence/2026-08-28-usbc-dock-900e-ramdump.md). |
