...
| Persistent direct boot | Reaches level-6 initcalls; timer path under investigation | A guarded Image-resident trace reached global initcall 524, level 6, and stopped inside `calibrate_xor_blocks()`. Its `do_xor_speed()` waits for `ktime_get()` to advance, matching the earlier RAID6 timing-loop failure. The next image disables that benchmark through a default-on Kconfig switch and retains exact per-initcall tracing. |
