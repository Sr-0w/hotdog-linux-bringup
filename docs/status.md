...
| Cameras | Telephoto and CSID TPG stream; CAMNOC rejects VFE DMA | Revisions `r51` through `r81` add SM8150 CAMSS/CCI, board wiring, the Samsung S5K3M5 driver, correct CSIPHY 1.1 programming and diagnostics. The telephoto emits complete RAW10 frames at 30 fps. The independent CSID generator also produces frame interrupts with the physical sensor and CSIPHY disconnected, but every userspace buffer retains 100 percent of its `0xAA` prefill. This rules out the sensor, CCI, MIPI link and CSIPHY. A read-only hardware register capture found a latched CAMNOC `Disconnected target` write error at the exact VFE buffer IOVA, `0x7ff80000`; the Apps SMMU stream match, translation context and global fault registers are valid. The leading gap is that downstream votes both the internal CAMNOC path and the CAMNOC-to-DDR path, while the current mainline CAMSS description votes only the latter. The three Sony sensors remain unimplemented; exploratory PM8009 rail enablement caused PMIC-level resets and is paused. See the [camera port plan](camera-port-plan.md). |
