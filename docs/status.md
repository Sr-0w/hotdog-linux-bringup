...
| Cameras | Blocked at the foundation | Three independent blockers. Mainline camss has no SM8150 backend at all: it covers sdm845 and sm8250 but not the SoC between them, so a CSIPHY/CSID/VFE port is needed first. None of the four sensors identified from the stock vendor modules has a mainline driver: Sony IMX586 main, IMX481 ultra-wide, Samsung S5K3M5 telephoto and Sony IMX471 in the pop-up. libcamera then depends on both. See the [feasibility survey](evidence/2026-08-07-mainline616-remaining-hardware.md). |
| Charging/battery | Charger limits validated; fuel gauge described but misreporting | The `r7` audit exposed unsafe SMB2 conversions on SMB5 and that build is superseded. Revision `r8` directly verifies raw PM8150B settings for 4.40 V float voltage, 1.50 A fast-charge current, and 500 mA USB input. Its 61-sample, 180-second trace remained below the 4.42 V guard. Revision `r48` enables `i2c8` and describes the BQ27541 fuel gauge, resolved to the eighth QUP serial engine through the stock overlay's own fixups. It probes as `bq27541-0` and reports a correct manufacturer, voltage and design capacity, but `capacity` reads 3842, `temp` reads -2211 and `cycle_count` returns exactly what `charge_now` returns, so the register offsets are shifted and the exact variant is still to be identified. Cable transitions, charge termination, low state of charge, thermal handling, suspend, and long-duration stability remain open. |
