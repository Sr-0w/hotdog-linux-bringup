...
| SLPI infrastructure | Working | SLPI boot, FastRPC, writable Hexagon service, registry regeneration, QRTR, SSC requests, event subscriptions and ULog forensics work end to end. The served tree carries socinfo, the OnePlus `project_name` and `oppoVersion` entries, and pre-created numbered registry files to sidestep the missing `O_CREAT`. The board identity is now provisioned from `/proc/cmdline`, so `oppo_project` reads 19801 instead of 0. The blocker was the firmware version, not the configuration: `modem_b` carries `SLPI.HY.2.2-00121`, an OTA-updated build, while the config and registry pair with `00083` from OxygenOS 10.0.13. Swapping that one file took hardware sensors from 1 to 7 and SEE data types from 4 to 41. See [the firmware version was the cause](evidence/2026-08-23-the-firmware-version-was-the-cause.md). |
| Motion/light sensors | Partial | Seven of eight stream, and were validated by hand rather than by byte counts: accelerometer (gravity on Z lying flat, then on Y held upright), gyroscope (reacts to rotation and settles), MMC5603 magnetometer (plausible field, responds to movement), IMU temperature, ambient light (reacts to occultation), plus SAR and motion detect on change. Use [the guided test](../helpers/hotdog-sensor-check.py), which states each gesture and validates itself. Proximity is the exception, below. |
...
| Proximity | Broken | `sns_tcs3701` publishes a proximity SUID and a full attribute set, accepts every request, and emits nothing: no sample, no configuration event, no error. Eliminated by measurement, each with the served registry read back to confirm the change took: `is_dri`, `hw_id`, subscription order, contention with another SEE client, and the calibration override. The driver runs — it rewrites `tcs3701_platform.prox.fac_cal` with a zero result 21 seconds into every boot, which is what a failed offset calibration stores. The chip is not the problem either: covering the sensor roughly triples the ALS channels on the same die, so the infrared emitter is driven and the finger is seen. What remains is inside the driver, between accepting the request and emitting, and its own messages for that region go to diag, which this port has no transport for. See [the driver runs and writes back zero](evidence/2026-08-23-the-proximity-driver-runs-and-writes-back-zero.md). |
