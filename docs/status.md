...
| Cameras | Telephoto RAW10 capture working; remaining sensors and libcamera open | Revisions `r51` through `r82` add SM8150 CAMSS/CCI, board wiring, the Samsung S5K3M5 driver, CSIPHY 1.1 support and diagnostics. A controlled CSID TPG capture isolated the last failure to a CAMNOC `Disconnected target` write at the exact VFE buffer IOVA while the Apps SMMU remained fault-free. Revision `r83` requests the downstream-required `GCC_CAMERA_HF_AXI_CLK`, which closes that bridge: the TPG writes all 384,000 bytes and a clean-boot S5K3M5 run captures three complete 16,423,680-byte 4208×3120 RAW10 buffers with distinct hashes. A decoded Bayer preview contains a real optical scene, and all CAMNOC error-log registers remain zero after capture. Telephoto raw capture is therefore hardware-validated end to end. The inferred IMX586, IMX481 and IMX471 sensors, safe PM8009 rail handling, libcamera tuning, automatic controls and camera-app integration remain open. See the [camera evidence](evidence/2026-08-09-mainline616-camera-telephoto.md) and [port plan](camera-port-plan.md). |
...
paths, sensors, remaining cameras, camera userspace integration, and suspend. Temporary DMA and bootloader-overlay
