...
| USB host/dock | Port managed and dual-role capable; host attachment not validated | Revision `r42` enables the PM8150B Type-C block with a dual-role connector, enables its VBUS boost, switches the controller to `otg` with a role switch, and terminates both connector endpoints. This was a device-tree gap only; every required kernel option was already set. `/sys/class/typec/port0` now exists, negotiates sink and device against a host PC, detects the partner, reports PD 3.0, and leaves the VBUS boost idle, while USB networking is unregressed. Attaching a peripheral or dock with the handset as host is still untested, as are SuperSpeed and DisplayPort alternate mode, which stay out of scope while the QMP PHY is disabled. |
