...
| Mainline 6.16 pmaports image | Source package direct hardware boot working; clean rootfs installed | Revision `r17` established the accepted display and userspace stack. Revision `r22` completes the stock memory reservation union; two strict builds produced the same 25,537,630-byte package. The source-built package is installed and its standard `boot-deploy` output direct-boots as `#23-oneplus-hotdog-mainline616`. A separate pinned clean worktree rebuilds the public kernel, firmware, and device packages and assembles a filesystem-checked Plasma Mobile image. Its expanded 14,002,683,904-byte image was written to unused `super` and read back to SHA256 `4eeac156cca4a2bbe5f70074901d5067c561a6cbdfae9d69574fdf5fa40d7f88`; the matching boot image still awaits its first hardware boot. |
...
| Persistent direct boot | Exact pmaports rootfs and networking working | The package-generated image boots from persistent `boot_b`, enumerates UFS, mounts the matching nested `pmOS_root` on `/dev/loop0p2`, completes `switch_root`, and starts OpenRC, USB networking, and SSH in 18 seconds. The accepted development GPT remains in `userdata`; the clean public-image GPT is independently staged in `super` for its first paired boot. |
...
| Plasma Mobile | Working on the direct-mainline system; clean replacement rootfs staged | The live root runs Plasma Mobile 6.7.3 through `tinydm`, with a usable full-screen shell, accelerated KWin, touch input, USB SSH, Discover/Flatpak, and smooth 0 A.D. gameplay. Device package `3-r6` selects the standard mobile application set and `polkit-elogind`, then disables PowerDevil automatic suspend in all three profiles until resume is reliable. The clean image contains 1,523 packages and the complete Plasma Mobile extras set; its verified `super` installation now awaits the matching clean boot image. |
