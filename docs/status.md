...
| Device package metadata | Complete image hardware-validated | The version-2 metadata and `kernel-cmdline.conf` generated the pmaports image used on hardware. Its 397-byte command line removes `quiet`/Plymouth, keeps `iommu.passthrough=0`, selects `TER16x32`, and boots successfully. Native 1440x3120 KMS scanout is now validated; the final package should add the appropriate DRM metadata while keeping 60 Hz as the accepted initial mode. |
...
| OpenRC userspace | Working directly | The exact pmaports rootfs starts OpenRC, NetworkManager, `sshd`, `pd-mapper`, and `tqftpserv`; host SSH is stable. |
| Plasma Mobile | Working on the direct-mainline system | The live root installed Plasma Mobile 6.7.3 and its postmarketOS OpenRC integration. The packaged `tinydm` path creates an active local seat, starts KWin, Xwayland, and `plasmashell`, and presents a usable full-screen mobile session while USB SSH remains available. This package selection still needs to be reproduced in a freshly assembled image. |
...
| DRM/panel | Working at 60 Hz | Native SM8150 DPU, DSI, TE, DSC, and the OnePlus Samsung panel produce correct 1440x3120 KMS scanout. `kmscube`, Weston, and Plasma Mobile show correct geometry without duplicated rows. Dense fbcon scrolling can still repeat; 90 Hz, blank/unblank, and suspend/resume remain open. |
...
| Firmware packages | Packaging complete, GPU runtime validated | `firmware-oneplus-hotdog` `20241212-r0` produces eight APKs and 16 payloads under `/usr/lib/firmware`. The A630 SQE, A640 GMU, and handset ZAP payloads are now runtime-validated. Other payloads and redistribution approval remain separate requirements. |
...
the S6SY761 touchscreen, Adreno 640 render path, native 60 Hz scanout, and
Plasma Mobile session are complete at basic runtime level, and both volume-key
devices now register. The remaining queue is extended charging policy,
physical key validation, 90 Hz, Wi-Fi/Bluetooth, audio, modem/remoteproc,
sensors, cameras, and suspend. Temporary DMA and
