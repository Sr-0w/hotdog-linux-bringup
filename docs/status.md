...
| Mainline 6.16 pmaports image | Clean public-tree image direct-boot working | Revision `r22` completes the stock memory reservation union. A pinned clean worktree rebuilds only the published kernel, firmware, and device packages and assembles a filesystem-checked 1,523-package Plasma Mobile image. Its expanded 14,002,683,904-byte image was written to unused `super` and read back to SHA256 `4eeac156cca4a2bbe5f70074901d5067c561a6cbdfae9d69574fdf5fa40d7f88`. Revision `r24` retains that rootfs and automatic AVB path, passed a complete `boot_b` readback, and direct-boots as `#25-oneplus-hotdog-mainline616` with ADSP and WCD9340 transport enabled. |
...
| Kernel modules | Exact `r24` package installed and running | Package `linux-oneplus-hotdog-mainline616-6.16.0-r24` installs the 6.16 module tree under `/usr/lib/modules/6.16.0-sm8150`. Its source-built kernel direct-boots and loads the packaged S6SY761, ath10k, Bluetooth, QRTR, MPSS, ADSP, SLIMbus, WCD9340, and SoundWire modules after `switch_root`. MSM DRM and Adreno are built in. |
...
| Audio | ADSP, APR, SLIMbus, WCD9340, and SoundWire transport working; no card yet | Revision `r24` keeps ADSP in `running`, exposes APR services 3, 4, 7, and 8, registers `171c0000.slim-ngd`, enumerates both WCD9340 logical devices, identifies codec revision `0x108.1`, and binds the codec, GPIO, and SoundWire children. Q6ASM still reaches `No dais found in DT` because the machine topology is intentionally absent. `/proc/asound/cards` remains empty; routing, headset thresholds, external TFA98xx amplifiers, and physical audio paths remain open. |
