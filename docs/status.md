...
| UFS | Direct boot working; buffered-import stability under investigation | Raw 1 GiB I/O and 120 seconds of direct random I/O pass with runtime power forced on. The same buffered OSTree object enters Qualcomm `900e` on `r18` with translated DMA64, `r19` with translated DMA32, and `r20` with legacy hardirq completions. No run contains a Linux panic or UFS/ext4 error. The next exact-image A/B limits `sda` requests to 128 KiB and captures all 8 GiB of DDR through Sahara on failure. |
