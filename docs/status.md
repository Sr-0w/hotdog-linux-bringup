...
| Cameras | Telephoto sensor streams to the CSID; VFE DMA output is still missing | Revisions `r51` through `r64` add the SM8150 CAMSS/CCI resources, board wiring, Samsung S5K3M5 driver and the correct CSIPHY 1.1 programming. The telephoto sensor probes and emits RAW10 at 4208x3120 and exactly 30 fps. CSIPHY and CSID error counters are clean, CSID byte counters reach the complete 16,411,200-byte payload, and VFE0 receives start-of-frame and write-master interrupts. The final userspace buffers nevertheless retain 100 percent of a diagnostic `0xAA` prefill, proving that the VFE writes no image data. DMA range, SMMU mappings, image addresses, geometry, clocks, power domains, interconnect voting, bus clock gating, CSID decoding and a minimal CAMNOC QoS sequence have been tested without changing that result. The next discriminator is the CSID generation-2 color-bar generator, using its only functional pattern (`test_pattern=9`) while leaving the CSID-to-VFE link enabled. The three Sony sensors remain unimplemented; exploratory PM8009 rail enablement caused PMIC-level resets and is paused. See the [camera port plan](camera-port-plan.md). |
