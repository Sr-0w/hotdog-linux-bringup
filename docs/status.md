...
| Thermal / suspend | Broken | A real `s2idle` cycle now enters and returns. The MPSS watchdog fires on every cycle where the modem runs, including with IPA in normal autosuspend; where it sits relative to resume is unresolved, since printk timestamps freeze across s2idle; the `ipa-clock-query` hypothesis is refuted, since that interrupt never fires at all. Wi-Fi does not survive resume and needs an `ath10k_snoc` reload. Cycle truncation follows a modem crash, not the charger. See [suspend/resume defects](evidence/2026-08-17-suspend-resume-defects.md). |
