...
| Device tree | Bring-up quality | Boots with temporary SMMU and ICE workarounds. Full-DDR analysis found that the mainline DT also omitted 768 KiB from the stock XBL/AOP reservation; `r21` restores it as `no-map`. |
| UFS | Direct boot working; buffered-import fix prepared | Raw 1 GiB I/O and 120 seconds of direct random I/O pass. The same buffered OSTree object enters Qualcomm `900e` on `r18` through `r20`, including with 128 KiB requests. A complete DDR dump found no UFS command, reset, recovery, or error in progress; instead, the file cache occupied firmware-owned memory omitted from the DT. The exact-image `r21` A/B reserves that range and awaits hardware validation. |
...
| RAM | Direct map working; reservation fix prepared | Direct boot receives the bootloader's multi-gigabyte memory map. Under pressure, `r20` allocated two file-cache folios across the omitted stock range `0x85e40000-0x85f00000`; `r21` excludes it. The historical kexec payload deliberately uses the low-bank window. |
