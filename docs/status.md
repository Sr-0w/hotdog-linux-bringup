...
| Mainline 6.16 pmaports image | Clean public-tree image direct-boot working | Revision `r22` completes the stock memory reservation union. A pinned clean worktree rebuilds only the published kernel, firmware, and device packages and assembles a filesystem-checked 1,523-package Plasma Mobile image. Its expanded 14,002,683,904-byte image was written to unused `super` and read back to SHA256 `4eeac156cca4a2bbe5f70074901d5067c561a6cbdfae9d69574fdf5fa40d7f88`. Revision `r25` retains that rootfs and automatic AVB path, passed a complete `boot_b` readback, and direct-boots as `#26-oneplus-hotdog-mainline616` with the internal ALSA machine card enabled. |
...
| Kernel modules | Exact `r25` package installed and running | Package `linux-oneplus-hotdog-mainline616-6.16.0-r25` installs the 6.16 module tree under `/usr/lib/modules/6.16.0-sm8150`. Its source-built kernel direct-boots and loads the packaged S6SY761, ath10k, Bluetooth, QRTR, MPSS, ADSP, SLIMbus, WCD9340, SoundWire, Q6 DAI/routing, and SM8150 machine modules after `switch_root`. MSM DRM and Adreno are built in. |
...
| Audio | Internal ALSA card and playback/capture PCM devices working; physical routes pending | Revision `r25` keeps ADSP and MPSS in `running`, binds `snd-sm8150` to `/sound`, and exposes card 0 `OnePlus 7T Pro` with `MultiMedia1` playback and capture. SLIMbus, WCD9340, and SoundWire remain bound. The unconfigured card correctly reports that no backend DAI route is selected; UCM routing, headset thresholds, external TFA98xx amplifiers, and physical speaker, earpiece, microphone, and headset tests remain open. |
