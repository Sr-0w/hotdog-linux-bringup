...
| Thermal / suspend | Broken | A real `s2idle` cycle now enters and returns. The MPSS watchdog fires 4.46 s *into* s2idle, not on resume, whenever IPA is active; the `ipa-clock-query` hypothesis is refuted, since that interrupt never fires at all. Wi-Fi does not survive resume and needs an `ath10k_snoc` reload. Cycle truncation follows a modem crash, not the charger. See [suspend/resume defects](evidence/2026-08-17-suspend-resume-defects.md). |
