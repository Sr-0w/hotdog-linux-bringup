...
| Mainline 6.16 pmaports image | Source package direct hardware boot working; clean full image built | Revision `r17` established the accepted display and userspace stack. Revision `r22` completes the stock memory reservation union; two strict builds produced the same 25,537,630-byte package. The source-built package is installed and its standard `boot-deploy` output direct-boots as `#23-oneplus-hotdog-mainline616`. A separate pinned clean worktree rebuilds the public kernel, firmware, and device packages and assembles a filesystem-checked Plasma Mobile image. Its raw boot/root pair still awaits the normal-install hardware test. The tested AVB image is `83ffc265b3f46042b840caa43d653dbbe2d1f8dd893169884ee7efcee9584b5d`. |
...
| Plasma Mobile | Working on the direct-mainline system; clean replacement image built | The live root runs Plasma Mobile 6.7.3 through `tinydm`, with a usable full-screen shell, accelerated KWin, touch input, USB SSH, Discover/Flatpak, and smooth 0 A.D. gameplay. Device package `3-r6` selects the standard mobile application set and `polkit-elogind`, then disables PowerDevil automatic suspend in all three profiles until resume is reliable. The clean image contains 1,523 packages and the complete Plasma Mobile extras set; its hardware installation remains pending. |
