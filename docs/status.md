...
| Motion/light sensors | Broken | LSM6DSM, MMC5603x and TCS3701 publish no physical SUID. Both tested firmware sets reject the QUP1/QUP2-to-EBI1 ICB routes with `ICBARB_ERROR_NO_ROUTE_TO_SLAVE`. Since 2026-08-19 the SLPI at least boots again: `slpi.mbn` was never packaged, so the sensor DSP had been offline on every boot, and the image rebuilt from the handset's `modem` partition now starts it. Holding the SLPI's `lcx`/`lmx` proxy domains, which downstream does with `qcom,keep-proxy-regs-on` and mainline now does through the PAS proxy patch, changes nothing: the sensor core still answers `no SUID`. The next control remains the current firmware/userspace on downstream 4.14 with stock DTBO. |
