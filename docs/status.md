...
| Mainline 6.16 pmaports image | Clean public-tree image direct-boot working | Revision `r22` completes the stock memory reservation union. A pinned clean worktree rebuilds only the published kernel, firmware, and device packages and assembles a filesystem-checked 1,523-package Plasma Mobile image. Its expanded 14,002,683,904-byte image was written to unused `super` and read back to SHA256 `4eeac156cca4a2bbe5f70074901d5067c561a6cbdfae9d69574fdf5fa40d7f88`. Its matching 96 MiB AVB image passed complete `boot_b` readback and direct-boots as `#23-oneplus-hotdog-mainline616` with the clean UUIDs mounted from `super`. A 151-sample, 300-second first-boot observation completed without a USB transition. A separate clean 1,524-package `3-r7` install now generates its final AVB image automatically and reproducibly, but has not been flashed. |
...
| Kernel modules | Exact `r22` package installed and running | Package `linux-oneplus-hotdog-mainline616-6.16.0-r22` installs the 6.16 module tree under `/usr/lib/modules/6.16.0-sm8150`. Its source-built kernel direct-boots and loads the packaged S6SY761, ath10k, Bluetooth, QRTR, and remoteproc modules after `switch_root`. MSM DRM and Adreno are built in. |
