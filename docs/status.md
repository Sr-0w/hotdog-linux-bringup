...
| UFS | Direct boot working; buffered-import stability under investigation | V33 enumerates the Samsung UFS with a conditional 32-bit DMA mask while Apps SMMU is bypassed. Raw 1 GiB I/O and a 120-second direct random-I/O test pass with runtime power forced on. The same multi-gigabyte buffered OSTree object reproducibly enters Qualcomm `900e` on `r18` with translated DMA64 and on `r19` with translated DMA32, without a Linux panic or UFS/ext4 error. Revision `r20` keeps the `r19` SMMU/DMA contract and restores legacy UFS completion handling to hardirq context for the next hardware A/B. |
