...
| Persistent direct boot | SMP bring-up under diagnosis | D26 exhausted all attempts immediately before `bringup_nonboot_cpus()`, while D25 did not reach the checkpoint after it with `maxcpus=1`. D27 and D28 show that boot-image `maxcpus=0` does not reach either later checkpoint. D29 forces the value in-kernel. |
...
27. Test D29 with `setup_max_cpus = 0` forced in the kernel before the call. A
   reset loop proves the prior command-line path was ineffective and provides
   a deterministic single-CPU bypass for continued bring-up.
28. Defer the r4 package-generated direct image until a direct handoff baseline
   works. Record its kernel, installed DTB, raw-image, and AVB hashes without
   reusing the historical r0 identity.
29. After a direct mainline entry succeeds, test the hotdog-only PON reboot-mode
