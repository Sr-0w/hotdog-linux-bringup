...
| Persistent direct boot | Reaches level-6 initcalls; guarded system-counter test prepared | A guarded Image-resident trace reached global initcall 524, level 6, and stopped inside `calibrate_xor_blocks()`. Its `do_xor_speed()` waits for `ktime_get()` to advance, matching the earlier RAID6 timing-loop failure. A first pre-MMU `CNTCR` probe held at the fixed logo without reaching automatic recovery. Its replacement arms the watchdog first, then records and enables `CNTCR` from mapped kernel context. The timer hypothesis remains unproven. |
