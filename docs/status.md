...
| Mainline 6.16 pmaports image | Clean public-tree image direct-boot working | Revision `r22` completes the stock memory reservation union. A pinned clean worktree rebuilds only the published kernel, firmware, and device packages and assembles a filesystem-checked 1,523-package Plasma Mobile image. Its expanded 14,002,683,904-byte image was written to unused `super` and read back to SHA256 `4eeac156cca4a2bbe5f70074901d5067c561a6cbdfae9d69574fdf5fa40d7f88`. Revision `r26` retains that rootfs and automatic AVB path, passed a complete `boot_b` readback, and direct-boots as `#27-oneplus-hotdog-mainline616` with the Hotdog digital headphone backend enabled. |
...
| Device package metadata | Hardware-validated; automatic AVB built offline | The version-2 metadata and `kernel-cmdline.conf` generated the pmaports image used on hardware. Its command line removes `quiet`/Plymouth, keeps `iommu.passthrough=0`, selects `TER16x32`, and boots successfully. Native 1440x3120 KMS scanout and dynamic selection between stock 60 Hz and 90 Hz are hardware-validated. Device package `3-r7` adds a verified `boot-deploy` postprocess hook that emits the required fixed-size hotdog AVB image during a normal install. |
| Persistent direct boot | Clean public rootfs and networking working | The package-generated image boots from persistent `boot_b`, enumerates UFS, maps physical `super` to `/dev/loop0`, mounts the clean `pmOS_root` on `/dev/loop0p2`, completes `switch_root`, and starts OpenRC, USB networking, SSH, and Plasma. The accepted development GPT remains intact in `userdata` as a fallback. |
| Device tree | Bring-up quality; stock RAM ownership validated | Boots with temporary SMMU and ICE workarounds. Full-DDR analysis found three gaps between mainline and the stock HD1913 reservation union. `r21` fixed XBL/AOP; `r22` excludes the remaining `removed_regions` and CDSP intervals and passed the reproducer on hardware. |
| UFS | Direct boot and large buffered imports working | Raw 1 GiB I/O and 120 seconds of direct random I/O pass. The buffered OSTree object entered Qualcomm `900e` through `r21` because Linux allocated stock-owned memory omitted from the DT. With the complete `r22` reservation union, the same 0 A.D. pull wrote 5,512,437,760 block-layer bytes, deployment completed, and the installed game now runs smooth interactive gameplay without UFS, ext4, panic, oops, or Qualcomm-transition evidence. |
| postmarketOS root | Working directly | The active clean pmaports initramfs selects UUID `0531b306-49b7-48ff-ac31-d5773e12f7b1`, expands it to the available space, mounts `/dev/loop0p2` read-write, and completes `switch_root`. |
| postmarketOS boot | Working directly | The matching active `pmOS_boot` UUID is `e409622e-17bb-484a-a813-7cb4b01fb56e` and is mounted read-write from `/dev/loop0p1`. |
...
| Plasma Mobile | Working in the clean public-tree image | Plasma Mobile 6.7.3 runs through `tinydm` with accelerated KWin, touch input, USB SSH, Discover/Flatpak, and the complete mobile application set. KWin scans out a 1440x3120 AR30 buffer at 90 Hz from the clean 1,523-package installation. Device package `3-r6` selects `polkit-elogind` and disables automatic suspend in all three PowerDevil profiles until resume is reliable; `3-r7` retains that policy and adds automatic AVB generation. Smooth 0 A.D. gameplay is validated on the active clean public-tree image. |
...
| DRM/panel | Fixed 60 Hz stable; 90 Hz selection works with resume defects | Native SM8150 DPU, DSI, TE, DSC, and the OnePlus Samsung panel produce correct 1440x3120 KMS scanout. Revision `r17` exposes both stock modes and sends the matching panel command during atomic changes. KScreen and Plasma Settings selection work. A later 90 Hz wake produced repeated DSI status 5 reports and an abnormal scanout; selecting 60 Hz restored the display according to direct observation while Linux and SSH remained healthy. Keep 60 Hz as the stable default until blank/unblank and resume sequences are corrected. |
...
| RAM | Direct map and complete stock reservation union working | Direct boot receives the bootloader's multi-gigabyte memory map. Under pressure, `r20` occupied the omitted XBL/AOP interval and `r21` occupied the next omitted CDSP interval. The `r22` A/B image reserves every enabled stock HD1913 interval and passed the exact pressure reproducer. Two strict package builds are byte-identical. The historical kexec payload deliberately uses the low-bank window. |
...
| Kernel modules | `r26` boot payload running with the ABI-compatible `r25` module tree | The direct-booted `r26` kernel differs from `r25` only in the source-built DTB. Package `linux-oneplus-hotdog-mainline616-6.16.0-r25` still supplies the unchanged 6.16 modules under `/usr/lib/modules/6.16.0-sm8150`; S6SY761, ath10k, Bluetooth, QRTR, MPSS, ADSP, SLIMbus, WCD9340, SoundWire, Q6 DAI/routing, and the SM8150 machine driver all load after `switch_root`. Persistent package replacement with `r26` remains pending. |
| Reboot | Clean software reboot working | Revision `r108` adds the missing SM8150 TCSR download-mode syscon and connects it to Qualcomm SCM. Six consecutive software reboots returned to direct-booted postmarketOS, USB NCM and SSH without entering Qualcomm `05c6:900e`. The exact R107 RAM dump showed orderly shutdown and no Linux panic; see the [software-reboot evidence](evidence/2026-08-10-mainline616-software-reboot.md). |
...
| Audio | Internal speakers and handset microphone working through Plasma | Revision `r32` matches the OxygenOS QUAT MI2S S24_LE format, 3.072 MHz BCLK, and TFA9874 slot profile. Independent channel captures validate the upper slot-0 and lower slot-1 speakers. Device package `3-r8` supplies a minimal speaker-only UCM profile, and a regular PulseAudio playback produces an independently measured approximately -46.8 dBFS 1 kHz component. Sink suspension closes the PCM and powers down both codecs; Plasma volume changes are software-only. Revision `r36` completes capture. A second `MultiMedia2` front end gives capture its own PCM, mapping the ports of every SLIMbus link rather than only the first ended a running stream of exact digital silence, and the stock overlay supplied the bias wiring: AMIC4 needs MIC BIAS1, not MIC BIAS4. A pad sweep against room sound identifies AMIC1, AMIC3 and AMIC4 as live and AMIC2 and AMIC5 as silent, so the stock label naming AMIC5 the handset microphone does not describe this handset; the odm mixer path naming amic4 does. Device package `3-r13` captures at `-37.1` dBFS RMS with `5.28` dB of level variation from the packaged profile alone, confirmed by listening. Note that playing a tone on the device's own speakers cannot validate capture: it produced convincing frequency discrimination through internal coupling while a silent pad was selected. The other live pads, digital microphones, echo cancellation, a usable capture volume control, protection telemetry, conservative default volume, headphones, earpiece, and headset detection remain open. |
...
| Cameras | All four physical cameras capture through libcamera; automatic front pop-up lifecycle validated | Revisions `r51` through `r90` provide S5K3M5 RAW10 capture, libcamera automatic exposure, calibrated manual focus and Plasma Camera. Revisions `r98` through `r109` add Sony IMX586 C-PHY capture, metadata and autofocus. Revision `r111` adds the physical IMX481 ultra-wide and validates two 180-frame processed runs. Revisions `r112` through `r120` bind the Sony IMX471 front sensor, expose both MXM1120 Hall sensors, validate the full opening and closing courses, capture a correctly exposed `1748x1740` image in Plasma Camera, and prove libcamera automatic exposure/gain over 180 metadata-only frames. Revisions `r121` through `r131` connect the Hall-bounded motor to IMX471 runtime PM, match the production OxygenOS cadence, clear the Hall endpoint plateau with a bounded motion probe, and validate automatic extension, 60-frame capture at approximately 90 fps, and immediate retraction on release. Device package `3-r17` keeps WirePlumber from owning CAMSS; `3-r18` installs and enables `qbootctl`. CAMSS recovery without reboot, AF/AE/AWB convergence, touch-to-focus, production color calibration and broader application testing remain open. See the [IMX471 and pop-up evidence](evidence/2026-08-10-mainline616-camera-imx471-popup.md), [IMX481 evidence](evidence/2026-08-10-mainline616-camera-imx481.md), [autofocus evidence](evidence/2026-08-10-mainline616-camera-autofocus.md), [A/B evidence](evidence/2026-08-10-ab-slot-success.md), [IMX586 evidence](evidence/2026-08-09-mainline616-camera-imx586.md), [main focus evidence](evidence/2026-08-10-mainline616-camera-imx586-focus.md), [telephoto capture evidence](evidence/2026-08-09-mainline616-camera-telephoto.md), [telephoto focus evidence](evidence/2026-08-09-mainline616-camera-telephoto-focus.md), [libcamera evidence](evidence/2026-08-09-mainline616-camera-libcamera.md) and [port plan](camera-port-plan.md). |
| Haptics | AW8697 hardware identified; driver package built | Runtime probing found the controller at `0x5a` and read chip ID `0x97`. Revision `r143` adds a device-tree binding, the stock-derived GPIO wiring and a minimal mainline-style `FF_RUMBLE` driver using the HD1913 170 Hz continuous-mode profile. Driver, DTB, schema and strict postmarketOS package builds pass. Physical vibration, stop/restart behavior and feedbackd integration have not yet been tested. See the [AW8697 evidence](evidence/2026-08-11-haptics-aw8697.md). |
| NFC | Controller and OxygenOS RF configuration validated; tag reading pending | The PN553-compatible controller binds through upstream NCI. Revision `r144` adds a `post_setup` loader and a packaged, reproducibly extracted 14-command OxygenOS RF/TVDD configuration. The kernel confirms that the complete configuration was applied, all advertised protocols poll and stop cleanly, GPIO 47 interrupts advance, and `neard` remains powered and polling. No target object has been reported yet. Explicit `DEV_DOWN` cannot recover without reboot. See the [NFC evidence](evidence/2026-08-10-nfc-nxp-nci.md). |
| Charging/battery | Charger limits and fuel gauge hardware-validated; extended policy open | The `r7` audit exposed unsafe SMB2 conversions on SMB5 and that build is superseded. Revision `r8` directly verifies raw PM8150B settings for 4.40 V float voltage, 1.50 A fast-charge current, and 500 mA USB input. Its 61-sample, 180-second trace remained below the 4.42 V guard. Revision `r50` enables `i2c8`, without GPI DMA and at 100 kHz as `i2c4` already needs, and describes the fuel gauge. The stock name is bq27541 but the registers are laid out as a bq27421: a raw dump with the driver unbound puts state of charge at `0x1c`, temperature at `0x02`, and remaining and full charge capacity at `0x0c` and `0x0e`. Described as `ti,bq27411` it reports correctly: 100 percent, 4.360 V, 35.1 C, and 3856 of 4040 mAh full charge capacity, which also gives a real state-of-health figure for the cell. Cable transitions, charge termination, low state of charge, thermal handling, suspend, and long-duration stability remain open. |
| USB host/dock | Host, SuperSpeed and DisplayPort working through a powered dock | Revision `r42` enables the PM8150B Type-C block with a dual-role connector, enables its VBUS boost, switches the controller to `otg` with a role switch, and terminates both connector endpoints. This was a device-tree gap only; every required kernel option was already set. `/sys/class/typec/port0` now exists, negotiates sink and device against a host PC, detects the partner, reports PD 3.0, and leaves the VBUS boost idle, while USB networking is unregressed. A powered Type-C dock enumerates through it: the port switches to `[host]` while staying a power sink, the xHCI root hubs come up, a GenesysLogic 4-port hub and a Realtek `0bda:8153` gigabit adapter enumerate, and `r8152` binds and creates `eth0`. No carrier was present because no cable was attached to the dock. Power delivery negotiates with the dock at the same time and the VBUS boost correctly stays off. Revision `r43` removes the USB 2.0-only description: the combo PHY and the DisplayPort controller are enabled and the connector declares a DisplayPort altmode. The same dock then enumerates its USB3.1 hub and gigabit adapter at 5000 Mbps, a SanDisk stick mounts and reads at its own 6.0 MB/s limit, and DisplayPort altmode negotiates `svid=ff01 active=yes` with `card0-DP-1` connected and enabled, EDID read, and a CRTC assigned. The Plasma desktop reaches the monitor. It is correct at `2560x1440@60`, where the driver trains 30 bpp for a 7.25 Gbps payload against an ~8.64 Gbps two-lane HBR2 budget. At `2560x1440@120` the picture is permanently corrupt: rather than pruning the mode, the driver degrades to 18 bpp and still exceeds the budget. That is an msm DP mode-validation defect worth reporting upstream, not a board description problem. Audio over DisplayPort does not work: revisions `r44` and `r45` add the missing back end and teach the machine driver the DAI id, so the `DISPLAY_PORT_RX` mixer controls exist and the spurious `invalid dai id` logging is gone, but the ADSP times out starting AFE port `0x6020` with `-110`. Revision `r46` additionally fixes an upstream bug worth submitting on its own: `DISPLAY_PORT_RX` is a receive port but was declared `SND_SOC_DAPM_AIF_OUT` while every other receive port uses `AIF_IN`. Everything under Linux control is now correct and verifiable, with `hdmi-audio-codec.0.auto` bound and registered as a card component, yet the port still will not start. The leading hypothesis is that the stock ADSP firmware does not provision the HDMI-over-DP port, since the handset never shipped DisplayPort output; video is unaffected because it never involves the DSP. That is not confirmed. Sourcing VBUS for an unpowered peripheral, HID, hotplug behaviour and four-lane DisplayPort remain untested. |
...
The clean image assembled from the published package directories now
direct-boots on hardware. Its kernel, source-built DTB, standard initramfs,
split installation, and deterministic AVB envelope were written with complete
readback verification; fresh SSH proved the clean kernel, filesystem UUIDs,
`super` backing, Plasma session, and hardware identities. Hardware enablement
can therefore proceed over the publication-shaped system. Device package
`3-r7` also closes the hidden manual AVB wrapping step: a full clean install
generated the verified 96 MiB envelope twice with identical bytes. The
remaining packaging gate is automatic `polkit-elogind` provider selection,
followed eventually by moving the hotdog footer contract into generic
...
fuel gauge, telephoto RAW10 capture, and powered-dock USB host/SuperSpeed/DisplayPort video paths are
...
paths, sensors, remaining cameras, telephoto autofocus/color calibration, and suspend. Temporary DMA and bootloader-overlay
