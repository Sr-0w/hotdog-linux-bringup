...
| Mainline 6.16 pmaports image | Clean public-tree image direct-boot working | Revision `r22` completes the stock memory reservation union. A pinned clean worktree rebuilds only the published kernel, firmware, and device packages and assembles a filesystem-checked 1,523-package Plasma Mobile image. Its expanded 14,002,683,904-byte image was written to unused `super` and read back to SHA256 `4eeac156cca4a2bbe5f70074901d5067c561a6cbdfae9d69574fdf5fa40d7f88`. Revision `r23` retains that rootfs and automatic AVB path, passed a complete `boot_b` readback, and direct-boots as `#24-oneplus-hotdog-mainline616` with ADSP enabled. |
...
| Kernel modules | Exact `r23` package installed and running | Package `linux-oneplus-hotdog-mainline616-6.16.0-r23` installs the 6.16 module tree under `/usr/lib/modules/6.16.0-sm8150`. Its source-built kernel direct-boots and loads the packaged S6SY761, ath10k, Bluetooth, QRTR, MPSS, and ADSP remoteproc modules after `switch_root`. MSM DRM and Adreno are built in. |
...
| Audio | ADSP and APR working; no sound card yet | Revision `r23` loads the 14,459,152-byte hotdog ADSP firmware, keeps the remote processor in `running`, and exposes APR services 3, 4, 7, and 8. Q6ASM reaches the expected `No dais found in DT` boundary because SLIMbus, WCD9340, the machine topology, and external TFA98xx amplifiers remain disabled. `/proc/asound/cards` is still empty. |
