...
| USB host/dock | Host mode working with a powered dock | Revision `r42` enables the PM8150B Type-C block with a dual-role connector, enables its VBUS boost, switches the controller to `otg` with a role switch, and terminates both connector endpoints. This was a device-tree gap only; every required kernel option was already set. `/sys/class/typec/port0` now exists, negotiates sink and device against a host PC, detects the partner, reports PD 3.0, and leaves the VBUS boost idle, while USB networking is unregressed. A powered Type-C dock enumerates through it: the port switches to `[host]` while staying a power sink, the xHCI root hubs come up, a GenesysLogic 4-port hub and a Realtek `0bda:8153` gigabit adapter enumerate, and `r8152` binds and creates `eth0`. No carrier was present because no cable was attached to the dock. Power delivery negotiates with the dock at the same time and the VBUS boost correctly stays off. Sourcing VBUS for an unpowered peripheral, mass storage, HID, and SuperSpeed plus DisplayPort alternate mode are still untested; the latter stay out of scope while the QMP PHY is disabled. |
