...
| Audio | Internal speakers and handset microphone working through Plasma | Revision `r32` matches the OxygenOS QUAT MI2S S24_LE format, 3.072 MHz BCLK, and TFA9874 slot profile. Independent channel captures validate the upper slot-0 and lower slot-1 speakers. Device package `3-r8` supplies a minimal speaker-only UCM profile, and a regular PulseAudio playback produces an independently measured approximately -46.8 dBFS 1 kHz component. Sink suspension closes the PCM and powers down both codecs; Plasma volume changes are software-only. Revision `r36` completes capture. A second `MultiMedia2` front end gives capture its own PCM, mapping the ports of every SLIMbus link rather than only the first ended a running stream of exact digital silence, and the stock overlay identified the handset microphone as AMIC5 on MIC BIAS1 rather than AMIC4. Three tones played on the validated speakers are each strongest in their own analysis bin by 16 to 46 dB, and the captured level tracks the analogue gain control step for step. Device package `3-r12` captures at `-41.3` dBFS RMS from the packaged profile alone. Headset and digital microphones, echo cancellation, a usable capture volume control, protection telemetry, conservative default volume, headphones, earpiece, and headset detection remain open. |
