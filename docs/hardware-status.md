...
Le `No memory resource` de simple-framebuffer a maintenant une hypothese concrete: le pack multi-DTB entry12 simplefb manquait `ranges;` sous `/chosen`; l'image preparee `024200` teste ce correctif avec le kernel pmaports configure pour VT/fbcon/simplefb et ajoute des diagnostics locaux Vol+/Vol-, tandis que `014400` reste le fallback qui garde le kernel stable.
