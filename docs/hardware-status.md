...
Le `No memory resource` de simple-framebuffer a maintenant une hypothese concrete: le pack multi-DTB entry12 simplefb manquait `ranges;` sous `/chosen`; l'image preparee `042500` teste ce correctif avec le kernel pmaports configure pour VT/fbcon/simplefb, les diagnostics locaux Vol+/Vol-, un follower tty0/tty1 dmesg/status avec preparation fb0/backlight en initramfs, et un shell/status follower tty1/tty0 apres `switch_root` pilotable aux boutons, tandis que `014400` reste le fallback qui garde le kernel stable.
