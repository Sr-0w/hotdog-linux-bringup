...
| D13 | Prepared: reset at the start of `__primary_switched` | Tests MMU enable, early kernel mapping and relocation, and the branch to the virtual kernel address. |
...
`__cpu_setup`, proving the complete pre-MMU path. D13 moves the checkpoint to
the first instructions at the MMU-on virtual entry. R6 plus
