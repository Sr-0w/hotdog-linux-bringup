...
| Direct DWC3 with D7 | Reproduced: 7/11 userspace cells | The diagnostic itself runs, but `/sys/bus/platform/devices/a6f8800.usb` is absent. Offline overlay replay proves D7 changes `/soc@0` from `2/2` to `1/1` cells without rewriting mainline child `reg` properties. |
| Guarded D3 DWC3 | Prepared, hardware validation pending | Preserve the embedded DTB with the D3 no-op overlay, build QCOM WDT in, add PM8150 Fastboot/recovery modes, and use a static 300-second supervisor with a two-second hardware fallback. |
