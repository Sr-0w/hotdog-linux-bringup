...
| D21 | Prepared: reset after `sched_init_smp()` | Separates SMP bring-up and scheduler SMP initialization from topology setup and the regular initcall sequence. |
...
`sched_init_smp()`, inside `kernel_init_freeable()`. R6 plus
