...
| D15 | Prepared: reset immediately before `start_kernel()` | Tests initial task and stack setup, vectors, FDT preservation, `kimage_voffset`, boot-mode state, and `finalise_el2`. |
...
virtual instructions and exhausted every slot retry. D15 moves the checkpoint
to the final assembly instruction before `start_kernel()`. R6 plus
